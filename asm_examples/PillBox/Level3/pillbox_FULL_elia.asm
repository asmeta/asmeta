...
definitions:
	...
	rule r_drawerOpened($drawer in Drawer) =
		par
			if redLedSwitch($drawer) != BLINKING and outMess($drawer) != ("Close " + drugName($drawer) + " in 10 minutes") then drawerTimer($drawer) := systemTime endif
		...
		forall $drawer in Drawer do
			if (drugIndex($drawer)<iton(length(time_consumption($drawer)))) then
				par					
					...
					if redLedSwitch($drawer) = BLINKING 
