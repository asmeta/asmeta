asm RushHourMonSimplForAsmetaSMV_EmptyAs0
...
definitions:
	...
	function carDir($car in Car) = at({0 -> NS, 1 -> NS, 2 -> EW, 3 -> NS, 4 -> NS, 5 -> NS, 6 -> NS, 7 -> EW, 8 -> EW,
										9 -> NS, 10 -> NS, 11 -> EW, 12 -> EW, 13 -> EW}, $car)
	...
	main rule r_Main =
		if(board(2,5) != 7) then
			let ($car=carMon, $dir=dirMon) in
				if($car != 0 and isMovePermitted($car, $dir))  then
					r_move[$car, $dir]
				endif
			endlet
		endif
