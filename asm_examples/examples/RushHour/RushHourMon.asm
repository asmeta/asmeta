...
definitions:
	...
	function isNextCellFree($car in Car, $dir in Dir) =
		switch($dir)
			case NORTH:
				(exists $r in Coord, $c in Coord with $r > 0 and board($r,$c) = $car and board(($r+5) mod 6,$c) = undef)
			case SOUTH:
				(exists $r1 in Coord, $c1 in Coord with $r1 < 5 and board($r1,$c1) = $car and board(($r1+1) mod 6,$c1) = undef)
			case WEST:
				(exists $r3 in Coord, $c3 in Coord with $c3 > 0 and board($r3,$c3) = $car and board($r3,($c3+5) mod 6) = undef)
			case EAST:
				(exists $r2 in Coord, $c2 in Coord with $c2 < 5 and board($r2,$c2) = $car and board($r2,($c2+1) mod 6) = undef)
		endswitch
	...
	rule r_move($car in Car, $dir in Dir) =
		forall $r in Coord, $c in Coord with board($r,$c) = $car do
			par
				if(board(($r + rMove($dir)) mod 6,($c + cMove($dir)) mod 6) = undef) then
	...
	//puzzle 40 (expert)
	//invariant inv_0 over board: size(< $r in Coord, $c in Coord | board($r,$c)=undef : board($r,$c)>)=7 //why the bag term requires bag domains as domains for the range variables?
	/*invariant inv_0 over board:  size({ $r in Coord, $c in Coord | board($r,$c)=undef : $r*6+$c})=7
	...
	//puzzle 3 (beginner)
	invariant inv_0 over board:  size({ $r in Coord, $c in Coord | board($r,$c)=undef : $r*6+$c})=22
	invariant inv_1 over board:  size({ $r in Coord, $c in Coord | board($r,$c)=redCar : $r*6+$c})=2
	...
	main rule r_Main =
		if not(redCarAtExit) then
			if isMovePermitted(carMon, dirMon) then
				r_move[carMon, dirMon]
			endif
		endif
