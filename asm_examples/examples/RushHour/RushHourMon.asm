...
definitions:
	...
	function isNextCellFree($car in Car, $dir in Dir) =
		switch($dir)
			case NORTH:
				(exist $r in Coord, $c in Coord with $r > 0 and board($r,$c) = $car and board(($r+5) mod 6,$c) = undef)
			case SOUTH:
				(exist $r1 in Coord, $c1 in Coord with $r1 < 5 and board($r1,$c1) = $car and board(($r1+1) mod 6,$c1) = undef)
			case WEST:
				(exist $r3 in Coord, $c3 in Coord with $c3 > 0 and board($r3,$c3) = $car and board($r3,($c3+5) mod 6) = undef)
			case EAST:
				(exist $r2 in Coord, $c2 in Coord with $c2 < 5 and board($r2,$c2) = $car and board($r2,($c2+1) mod 6) = undef)
		endswitch
