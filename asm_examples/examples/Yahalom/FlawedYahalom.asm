...
signature:
	...
	enum domain MessageType = {AN | ANANK | AKNNKAKK | AKKNK | ANK | AKNNK | AKK | NK}
	dynamic controlled messageType: Message -> MessageType

	dynamic controlled messageAN: Message -> Prod(Agent, Nonce)
	dynamic controlled messageAN_ANK: Message -> Prod(Agent, Nonce, Message)
	...
	dynamic controlled messageANK: Message -> Prod(Agent, Nonce, ShrKey)
...
definitions:
	...
	function decryptANK($messageANK in Message, $shrKey in ShrKey) =
		let ($message = messageANK($messageANK)) in
			...
			let ($traffic = traffic(receive(self))) in
				if(second($traffic)=self) then // il messaggio e' destinato all'agente
					let($receivedMessage = third($traffic)) in
						if(messageType($receivedMessage)=ANANK) then // e' un messaggio per questa regola
							let($y = first(messageAN_ANK($receivedMessage)),
									$n2 = second(messageAN_ANK($receivedMessage)),
									$msg = third(messageAN_ANK($receivedMessage)),
									$decrMsg = decryptANK($msg, shrkey($y)),
									$x = first($decrMsg), $n1 = second($decrMsg),
								...
								par
									extend Nonce with $newNonce do
										par
											...
											isFresh($newNonce) := true
											r_addNonce[self, $newNonce]
											extend Message with $messageAN_ANK, $messageANK  do
												par
													messageType($messageAN_ANK) := ANANK
													messageType($messageANK) := ANK
													messageAN_ANK($messageAN_ANK) := (self, $newNonce, $messageANK) // la nuova nonce non viene crittata
													messageANK($messageANK) := ($x, $n0, shrkey(self))
													r_send[self, server, $messageAN_ANK]
												endpar
										endpar
									r_clear[receive(self)]
								endpar
