...
//Op2 non viene soddisfatto. E' possibile eseguire il saldo un numero illimitato di volte.

//errore nella gestione di STANDARD e OTHER.
...
definitions:
	...
	macro rule r_prelievo =
		par
			if(atmState=STANDARDAMOUNTSELECTION) then
				if(exists $m in MoneySize with $m=insertMoneySize) then
	...
	main rule r_Main =
		seq
			r_goOutOfService[] //in sequenza perche', prima di servire un eventuale cliente, bisogna assicurarsi
								//che l'ATM non sia fuori servizio 
			par
				r_insertcard[]
				r_enterPin[]
				r_chooseService[]
