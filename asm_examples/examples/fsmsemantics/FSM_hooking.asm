// FSM semantics by semantic hooking
// resembles the spec given by semantic anchoring
...
//Gamma, defined by the user  
      ...
      //Functions on State
      controlled outTransitions: State -> Powerset(Transition)
...
definitions:
	...
	rule r_fsmReact($m in Fsm, $e in Event) = 
        ...
        choose $pt in outTransitions($s) with inputEvent($pt)= $e do r_doTransition[$m,$pt]          
        endlet
...
default init s0:
      ...
      //Functions on State
      function outTransitions($s in State)= at({s1->{t1,t2},s2->{t3,t4}},$s)
