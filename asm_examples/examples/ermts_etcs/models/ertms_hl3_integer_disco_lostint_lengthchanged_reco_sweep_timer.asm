asm ertms_hl3_integer_disco_lostint_lengthchanged_reco_sweep_timer
...
//	TRAIN States	
	...
	dynamic controlled risk_other_trains_in_this_vss: Prod(Trains, Vss) -> Boolean
	...
	macro rule  r_check_shadow_trains  =
		...
		forall $trains in Trains with true do  
		  if isDef(ttd_rear_train($trains)) then
			if train_has_crossed_ttd($trains) and vss_states(last_vss_onthe_ttd(ttd_rear_train($trains))) = AMBIGUOUS then
			   if ttd_states(ttd_rear_train($trains)) = TTD_FREE and trains_integer($trains) and shadow_train_timerA(ttd_rear_train($trains))  and shadow_train_timerB(ttd_rear_train($trains)) then
 ...
 //3.4.2.4    
    macro rule r_check_integrityloss_propagation_timer =
            ...
            forall $trains in Trains, $vss in Vss with train_inthis_vss($trains, $vss) do
                if vss_states($vss) = OCCUPIED then
                   if (lost_integrity($trains) or (wait_integrity_timer($trains)) or (length_changed($trains))) then
                      par
                ...
                else if (trains_confirmed_integrity($trains) and not(length_changed($trains))) or vss_states($vss) = FREE or vss_states($vss) = OCCUPIED then
    ...
    macro rule r_check_two_reporting_trains_inonevss =
    			     ...
    			     forall $tr in Trains, $trains in Trains with $tr != $trains and train_rear_another_train($tr, $trains) do    			    	
	         	        ...
	         	        risk_other_trains_in_this_vss($trains, vss_related_to_train_position(train_position($tr))) := true
	         	      endpar
	         	    else
	         	        risk_other_trains_in_this_vss($trains, vss_related_to_train_position(train_position($tr))) := false     
	         	    endif 	         	   
	...
	macro rule r_check_shadow_train_timer =
			...
			forall $ttd in Ttd, $trains in Trains with true do
			  switch($ttd)
			     case ttd_10:
			       par
			          if ttd_states(ttd_10) = TTD_FREE then 		            
			             if vss_prestates (vss_12) = AMBIGUOUS then 
				 ...
				 case ttd_20:
				   par
			          if ttd_states(ttd_20) = TTD_FREE then
			               if vss_prestates(vss_23) = AMBIGUOUS then
							  shadow_train_timerA($ttd) := true
						   endif
					  endif 
				 ...
				 case ttd_30:
				    par
			          if ttd_states(ttd_30) = TTD_FREE then
			               if (vss_prestates(vss_33) = AMBIGUOUS) then
							 shadow_train_timerA($ttd) := true
						endif
					  endif 
...
// 4.3
	macro rule r_check_loss_integrity_situations($trains in Trains) =	
		  ...
		  par			    
			...
			forall $vss in Vss with $vss = vss_rear_train($trains) do	 
			  	if integrityloss_propagation_timer(vss_rear_train($trains)) then
			          if ttd_states(vss_on_ttd($vss)) = TTD_OCCUPIED or not (train_inthis_vss($trains, $vss) and rbc_conn($trains)) or not ma_on_vss($vss, $trains) then
			            par   
			                vss_states($vss) := UNKNOWN
			                vss_prestates($vss) := vss_states($vss)
						endpar
...
// 3.5	Operation of trains treated as integer			
	...
	macro rule r_Trains_reconnecting($trains in Trains, $vss in Vss) =
			...
			if not mute_timer($trains) and vss_states($vss) = UNKNOWN and train_inthis_vss($trains, $vss) and contains(train_state($trains), NOTCONNECTED) then 
			  par 					  		  			     
			     ...
			     if ((trains_confirmed_integrity($trains)) and not(length_changed($trains)) and not(risk_other_trains_in_this_vss($trains, $vss))) then 	
	...
	macro rule r_Rbc_Supervisor  =
			...
			par 			   			   
			   ...
			   r_check_shadow_train_timer[]			//controllare non va la ttd_state			   			   		   			    			   			   	      
			endpar   
...
default init initial_state:
	...
	function risk_other_trains_in_this_vss($trains in Trains, $vss in Vss) = false
