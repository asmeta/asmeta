...
//	TTD States	
	...
	derived ttd_rear_ttd: Ttd -> Ttd
	derived ttd_advance_ttd: Ttd -> Ttd
...
// VSS related to the train position
	...
	function ttd_rear_ttd($ttd in Ttd) =
...
//4.2.2	
	...
	macro rule r_check_ghost_trains =
			...
			forall $ttd in Ttd, $trains in Trains  with true do
               if ghost_propagation_timer($ttd) and (not ghost_timer) then
                  par
                     ghost_propagation_timer($ttd) := false  
				     forall $vss in Vss with (vss_states($vss) = FREE or vss_states($vss) = UNKNOWN) and ttd_states(vss_on_ttd($vss)) = TTD_OCCUPIED and ma_on_ttd(vss_on_ttd($vss), $trains) do
					   par 
...
// 4.5.1.4	
			...
			if (train_has_crossed_ttd($trains) and vss_states(last_vss_onthe_ttd(ttd_rear_train($trains))) = AMBIGUOUS) or trains_shadow($trains) then
			   if ttd_states(ttd_rear_train($trains)) = TTD_FREE and not lost_integrity($trains) and shadow_train_timerA(ttd_rear_train($trains)) and shadow_train_timerB(ttd_rear_train($trains)) then 			   
			       r_vss_train_occupied[$trains] 
			          ...
			          forall $trains2 in Trains with $trains != $trains2 and train_rear_another_train($trains2, $trains) do 
			             if not trains_shadow($trains) and rbc_conn($trains) then
			                 r_vss_train_occupied[$trains]
			             else    
	...
	macro rule r_check_ma_of_chasing_trains =
		  ...
		  forall $trains in Trains with true do	
			if trains_chasing($trains) and train_speed($trains) > 0 then
			   choose $vss in Vss with  ma_on_vss($vss, $trains) and vss_states($vss) = UNKNOWN and train_position($trains) < start_vss_position($vss) do
			       par
	...
	macro rule r_check_ghost_propagation_timer  =
			...
			forall $trains in Trains, $ttd in Ttd with ghost_timer do
				if (ttd_states($ttd) = TTD_OCCUPIED and train_on_ttd($ttd, $trains) and not rbc_conn($trains)) or ((ttd_states($ttd) = TTD_OCCUPIED) and (not(ma_on_ttd($ttd, $trains)))) then
		  ...
		  forall $trains in Trains with true do	
			if not rbc_conn($trains) or lost_integrity($trains) or wait_integrity_timer($trains) or length_changed($trains) then 
			   ...
			   par
				  ...
				  if rbc_conn($trains) and contains(train_state($trains), NOTLOSTINTEGRITY) and contains(train_state($trains), NOTWAITINTEGRITYTIMER)  and contains(train_state($trains), NOTLENGTHCHANGED) then
...
// 3.5	Operation of trains treated as integer			
	macro rule r_Trains_integer ($trains in Trains) =
		...
		par   			
			if (not trains_shadow($trains)) and rbc_conn($trains) and trains_confirmed_integrity($trains) and (train_position($trains) <= second(ma($trains)) and not lost_integrity($trains)) then
			   par  
			     confirmed_safe_rear_end($trains) := train_position($trains) - train_length($trains) - min_safe_rear_end($trains)		   
				 if (first(train_prestate($trains)) = NOTLOSTINTEGRITY) and not(wait_integrity_timer($trains)) then 
  		            forall $tr in Trains with $tr != $trains do 
			            if not train_inthis_vss($tr, vss_related_to_train_position(train_position($trains))) and not trains_chased($trains)  then
			                 r_vss_train_occupied[$trains]     
			            endif                  
			     endif
			   endpar    
	...
	macro rule r_Trains_reconnecting($trains in Trains, $vss in Vss) =
			...
			if rbc_conn($trains) and vss_states($vss) = UNKNOWN and train_inthis_vss($trains, $vss) and contains(train_state($trains), NOTCONNECTED) then
			  par 					  		  			     
    ...
    macro rule r_Trains_connected ($trains in Trains) =
				...
				if rbc_conn($trains) then
	         		par
	         			confirmed_safe_rear_end($trains) := train_position($trains) - train_length($trains) - min_safe_rear_end($trains)    	                  	            	    
	            	   	 	mute_timer($trains) :=false
	            	   	 	if rbc_conn($trains) or contains(train_state($trains), NOTLOSTINTEGRITY) or contains(train_state($trains), NOTWAITINTEGRITYTIMER) then
	            	    		    trains_confirmed_integrity($trains) := true
	            	    		else
	            	    		    trains_confirmed_integrity($trains) := false
	            	    		endif       
...
default init initial_state:
	...
	function confirmed_safe_rear_end($trains in Trains) = 80
