...
//3.4.2.4    
    ...
    macro rule r_check_disconnect_propagation_timer =
    		       ...
    		       forall $vss in Vss, $trains in Trains with disconnect_propagation_timer($vss) and not rbc_conn($trains) do 
    		          par
	...
	macro rule r_check_trains_onttd =
	     		...
	     		forall $trains in Trains, $ttd in Ttd with ttd_states($ttd) = TTD_OCCUPIED   do
	     			if not train_on_ttd($ttd, $trains)  then
            				par 
            				   ttd_states($ttd) := TTD_FREE
					    	   ttd_prestates($ttd) := ttd_states($ttd)
					    	   r_check_shadow_train_timer[]
		  ...
		  forall $trains in Trains with true do	
		    par 
		  	   if rbc_conn($trains) then
		  	        forall $vss_inthe_trackside in Vss with disconnect_propagation_timer($vss_inthe_trackside) do 
...
// 3.5	Operation of trains treated as integer			
	...
	macro rule r_Trains_reconnecting($trains in Trains, $vss in Vss) =
			...
			if rbc_conn($trains) and vss_states($vss) = UNKNOWN and train_inthis_vss($trains, $vss) and not mute_timer_state($trains)   then 
			  par 					  		  			     
			     ...
			     if (train_position($trains) <= second(ma($trains))) and not (disconnect_propagation_timer(vss_advance_train($trains)) or disconnect_propagation_timer(vss_rear_train($trains)) or disconnect_propagation_timer(vss_related_to_train_position(train_position($trains)))) then 
			       if (isDef(vss_advance_train($trains))) and first(ma($trains)) = fourth(collect_trains_data($trains))  and second(ma($trains)) = fifth(collect_trains_data($trains)) then			        
			          par			   
			     ...
			     if (trains_confirmed_integrity($trains)) and not(length_changed($trains)) and not (disconnect_propagation_timer(vss_advance_train($trains)) or disconnect_propagation_timer(vss_rear_train($trains)) or disconnect_propagation_timer(vss_related_to_train_position(train_position($trains)))) then 	
				    if (isDef(vss_rear_train($trains))) and first(ma($trains)) = fourth(collect_trains_data($trains))  and second(ma($trains)) = fifth(collect_trains_data($trains)) then			        	  
					   par			             
			              vss_states(vss_rear_train($trains)) := FREE
			              vss_prestates(vss_rear_train($trains)) := vss_states(vss_rear_train($trains)) 		            	  
			           endpar
