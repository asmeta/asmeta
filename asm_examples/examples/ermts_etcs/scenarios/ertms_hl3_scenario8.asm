...
// VSS rear	train
	...
	function train_has_crossed_ttd($trains in Trains) = 	
	          ...
	          if (train_position($trains) < 50 and first(collect_trains_data($trains)) <  21) or 	
	          					(train_position($trains) < 80 and first(collect_trains_data($trains)) <  51 )	 then 
	                true
	          else
	          		false 
	          endif	
	                            ...
	                            par
	                                ...
	                                vss_prestates(vss_21) := vss_states(vss_21)                    
                          		   ...
                          		   if ghost_propagation_timer(ttd_10) then  
                          		      if (not (ttd_states(ttd_10) = TTD_FREE)) or vss_states(vss_12) = FREE  or ma_on_ttd(ttd_10, $trains) then
                          		          par
                          		          		vss_states(vss_12) := UNKNOWN  
                          		          		vss_prestates(vss_12) := vss_states(vss_12)                       
                          		   		  endpar	
...
// 4.3
	macro rule r_check_loss_integrity_situations($trains in Trains) =	
		  ...
		  par			    
			...
			if contains(train_prestate($trains), INTEGRITYCONFIRMED) and (contains(train_state($trains), LOSTINTEGRITY)  or contains(train_state($trains), LENGTHCHANGED) or contains(train_state($trains), WAITINTEGRITYTIMER)) then	
			...
			if trains_confirmed_integrity($trains) and train_position($trains) <= second(ma($trains))  and  rbc_conn($trains) and not trains_shadow($trains) then
			   par  
				 ...
				 if (first(train_prestate($trains)) = NOTLOSTINTEGRITY) and not(wait_integrity_timer($trains)) then 
  		            forall $tr in Trains with $tr != $trains do 
			            if not train_inthis_vss($tr, vss_related_to_train_position(train_position($trains))) and not (trains_chased($tr)  or trains_chasing($tr)) then
			                 r_vss_train_occupied[$trains]     
			            endif               
			     endif
			   endpar    
