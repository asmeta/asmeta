...
// set all trains vss to unknown			
    ...
    macro rule r_check_trains_chasing_chased =	
	          ...
	          forall $trains_in_advance in Trains, $trains in Trains with $trains != $trains_in_advance and train_rear_another_train($trains, $trains_in_advance) do
	            if train_position($trains) >= (train_position($trains_in_advance) - train_length($trains_in_advance) - confirmed_safe_rear_end($trains_in_advance)) then
	              par  
...
// 4.5.1.4	
	...
	macro rule r_check_ma_of_chasing_trains =
		  ...
		  forall $trains in Trains with true do	
			if trains_chasing($trains) and train_speed($trains) > 0 then
		  ...
		  forall $trains in Trains with true do	
			if not rbc_conn($trains) or lost_integrity($trains) or wait_integrity_timer($trains) or length_changed($trains) then 
			   ...
			   par
				  ...
				  if contains(train_state($trains), CONNECTED) and contains(train_state($trains), NOTLOSTINTEGRITY) and contains(train_state($trains), NOTWAITINTEGRITYTIMER)  and contains(train_state($trains), NOTLENGTHCHANGED) then
			   ...
			   par  
			     ...
			     if (first(train_prestate($trains)) = NOTLOSTINTEGRITY) and not(wait_integrity_timer($trains)) then  
  		            forall $tr in Trains with $tr != $trains and train_rear_another_train($tr, $trains) do
			            if not train_inthis_vss($tr, vss_related_to_train_position(train_position($trains))) or (train_inthis_vss($tr, vss_related_to_train_position(train_position($trains))) and rbc_conn($tr)) then
			                 r_vss_train_occupied[$trains]  
			            endif          
			     endif
			   endpar    
    ...
    macro rule r_Trains_connected ($trains in Trains) =
				...
				if rbc_conn($trains) then
	         		par
	         			confirmed_safe_rear_end($trains) := train_position($trains) - train_length($trains) - min_safe_rear_end($trains)    	                  	            	    
	            	   	 	mute_timer($trains) :=false
	            	   	 	if contains(train_state($trains), CONNECTED) or contains(train_state($trains), NOTLOSTINTEGRITY) or contains(train_state($trains), NOTWAITINTEGRITYTIMER) then
	            	    		    trains_confirmed_integrity($trains) := true
	            	    		else
	            	    		    trains_confirmed_integrity($trains) := false
	            	    		endif       
...
default init initial_state:
	...
	function trains_confirmed_integrity($trains in Trains) = switch($trains)
																case train2:	 true
																case train1: false
															endswitch	
	...
	function collect_trains_data($trains in Trains) =  switch($trains)
														  case train1: (20, 5, 0, 15, 70)
														  case train2: (15, 5, 0, 15, 40)
													  endswitch	  
	function collect_trains_predata($trains in Trains) = (0, 0, 0, 0, 0)  
	
	
	function train_state($trains in Trains) =  switch($trains)
													case train1: [NOTLOSTINTEGRITY,  NOTLENGTHCHANGED, CONNECTED, NOTSHADOWTRAIN, INTEGRITYCONFIRMED, NOTWAITINTEGRITYTIMER]
													case train2: [LOSTINTEGRITY,  NOTLENGTHCHANGED, NOTCONNECTED, NOTSHADOWTRAIN, NOTINTEGRITYCONFIRMED, WAITINTEGRITYTIMER]
											  endswitch		
