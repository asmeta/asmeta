...
// VSS rear	train
	...
	function train_inthis_vss($trains in Trains, $vss in Vss) =
			...
			if vss_related_to_train_position(train_position($trains)) = $vss then
				true
			else
				false	
	...
	function train_on_ttd($ttd in Ttd, $trains in Trains) =
			...
			if (ttd_related_to_absolute_position(train_position($trains) - train_length($trains) + 1) = position_related_to_ttd($ttd)) then
			        true
			else
					false
			endif	
...
//3.4.2.4    
	...
	macro rule r_check_disconnect_propagation_timer =
    		      ...
    		      forall $vss in Vss, $trains in Trains with not rbc_conn($trains)  do 
    		        if not disconnect_propagation_timer($vss) then   
    		            par
    		             forall $vss2 in Vss with position_related_to_vss($vss) <  position_related_to_vss($vss2) and ttd_states(vss_on_ttd($vss2)) = TTD_OCCUPIED do
                         par   
                      	    vss_states($vss2) := UNKNOWN	
                      	    vss_prestates($vss2) := vss_states($vss2)  	 
                      	endpar
                     forall $vss3 in Vss with position_related_to_vss($vss) >  position_related_to_vss($vss3) and ttd_states(vss_on_ttd($vss3)) = TTD_OCCUPIED  do 
                         par   
	...
	macro rule r_check_trains_onttd =
	     		...
	     		forall $trains in Trains, $ttd in Ttd, $t in Trains with ttd_states($ttd) = TTD_OCCUPIED  and $trains != $t do
	     			if not train_on_ttd($ttd, $trains) and not train_on_ttd($ttd, $t) then
            				par 
            				   ttd_states($ttd) := TTD_FREE
					    	   ttd_prestates($ttd) := ttd_states($ttd)
					    	   r_check_shadow_train_timer[]
            				   forall $vss in Vss with vss_on_ttd($vss) = $ttd do
            				      par
            				      	vss_states($vss) := FREE
					 			vss_prestates($vss) := vss_states($vss)
					 		  endpar	  		
...
//4.2.2	
	macro rule r_check_ghost_trains =	
	
	          forall $ttd in Ttd, $trains in Trains with true do
	              switch($ttd)
	                 case ttd_10: 
	                         if ttd_states(ttd_10) = TTD_OCCUPIED and vss_states(vss_11) = FREE and vss_states(vss_12) = FREE and (not ma_on_ttd(ttd_10, $trains)) then
	                            par
	                                vss_states(vss_11) := UNKNOWN      
	                                vss_prestates(vss_11) := vss_states(vss_11)                  
                          		   vss_states(vss_12) := UNKNOWN   
                          		   vss_prestates(vss_12) := vss_states(vss_12)                                            		    
                          		   if ghost_propagation_timer(ttd_20) then  
                          		        if (ttd_states(ttd_20) != TTD_FREE) or vss_states(vss_21) = FREE  or ma_on_ttd(ttd_20, $trains) then 
                          		          par
                          		          		vss_states(vss_21) := UNKNOWN    
												vss_prestates(vss_21) := vss_states(vss_21)                    
                          		   		  endpar	
                          		   		endif  	
                          		   	endif	 
                          	    endpar
                     ...
                     case ttd_20: 
	                         if ttd_states(ttd_20) =TTD_OCCUPIED and vss_states(vss_21) = FREE and vss_states(vss_22) = FREE and vss_states(vss_23) = FREE  and (not ma_on_ttd(ttd_20, $trains)) then
	                            par
	                                vss_states(vss_21) := UNKNOWN    
	                                vss_prestates(vss_21) := vss_states(vss_21)                    
                          		   vss_states(vss_22) := UNKNOWN   
                          		   vss_prestates(vss_22) := vss_states(vss_22)                     
                          		   vss_states(vss_23) := UNKNOWN   
                          		   vss_prestates(vss_23) := vss_states(vss_23)                    
                          		   if ghost_propagation_timer(ttd_30) then  
                          		      if (ttd_states(ttd_30) != TTD_FREE) or vss_states(vss_31) = FREE  or ma_on_ttd(ttd_30, $trains) then 
                          		          par
                          		          		vss_states(vss_31) := UNKNOWN   
                          		          		vss_prestates(vss_31) := vss_states(vss_31)                      
                          		   		  endpar	
                          		   	  endif 	  	
                          		   endif
                          		   if ghost_propagation_timer(ttd_10) then  
                          		      if (ttd_states(ttd_10) != TTD_FREE) or vss_states(vss_12) = FREE  or ma_on_ttd(ttd_10, $trains) then 
                          		          par
                          		          		vss_states(vss_12) := UNKNOWN  
                          		          		vss_prestates(vss_12) := vss_states(vss_12)                       
                          		   		  endpar	
                     ...
                     case ttd_30: 
	                         if ttd_states(ttd_30) = TTD_OCCUPIED and vss_states(vss_31) = FREE and vss_states(vss_32) = FREE and vss_states(vss_33) = FREE and (not ma_on_ttd(ttd_30, $trains)) then
	                            par
	                                vss_states(vss_31) := UNKNOWN  
	                                vss_prestates(vss_31) := vss_states(vss_31)                      
                          		   vss_states(vss_32) := UNKNOWN   
                          		   vss_prestates(vss_32) := vss_states(vss_32)                     
                          		   vss_states(vss_33) := UNKNOWN 
                          		   vss_prestates(vss_33) := vss_states(vss_33)                       
                          		   if ghost_propagation_timer(ttd_20) then  
                          		      if (ttd_states(ttd_20) != TTD_FREE) or vss_states(vss_23) = FREE  or ma_on_ttd(ttd_20, $trains) then 
                          		          par
                          		          		vss_states(vss_23) := UNKNOWN 
                          		          		vss_prestates(vss_23) := vss_states(vss_23)                        
                          		   		  endpar	
...
// 4.5.1.4	
	macro rule  r_check_shadow_trains  =
	
		forall $trains in Trains with true do  
		  if isDef(ttd_rear_train($trains)) and rbc_conn($trains) then
			if (train_has_crossed_ttd($trains) and vss_states(last_vss_onthe_ttd(ttd_rear_train($trains))) = AMBIGUOUS) or trains_shadow($trains) then
			   if ttd_states(ttd_rear_train($trains)) = TTD_FREE and not lost_integrity($trains) and shadow_train_timerA(ttd_rear_train($trains))  and shadow_train_timerB(ttd_rear_train($trains)) then 
	...
	macro rule r_check_ma_of_chasing_trains =
	
		  forall $trains in Trains with true do	
			if trains_chasing($trains) then
			   forall $vss in Vss with  true do
			      if ma_on_vss($vss, $trains) and vss_states($vss) = UNKNOWN and train_position($trains) < start_vss_position($vss) then
			           par
			         		shortened_ma($trains) :=  start_vss_position($vss)			           
	...
	// 4.4    				    
    macro rule r_check_two_reporting_trains_inonevss =
    
    			     forall $tr in Trains, $trains in Trains with $tr != $trains and train_rear_another_train($tr, $trains) do  	
    			   		if vss_related_to_train_position(train_position($tr)) = vss_related_to_train_position(train_position($trains) - train_length($trains) + 1) then
    			   		    if vss_states(vss_related_to_train_position(train_position($tr))) = OCCUPIED then
    			   			  par
    			   			     vss_states(vss_related_to_train_position(train_position($tr))) := AMBIGUOUS
    			   			     vss_prestates(vss_related_to_train_position(train_position($tr))) := vss_states(vss_related_to_train_position(train_position($tr)))        	             
	         	          endpar
	         	        endif
	         	    endif               	  	         	   
	...
	macro rule r_check_ghost_propagation_timer  =
			  			 
			forall $trains in Trains, $ttd in Ttd with true do
			    if (ttd_states($ttd) = TTD_OCCUPIED) and (not (train_on_ttd($ttd, $trains))) or ((ttd_states($ttd) = TTD_OCCUPIED) and (not(ma_on_ttd($ttd, $trains)))) then 
			         ghost_propagation_timer($ttd) := true 
			    endif       		
	
	macro rule r_check_rbc_trains_connection_collect_data = 
	
		  forall $trains in Trains with true do	
			if not rbc_conn($trains) or lost_integrity($trains) or wait_integrity_timer($trains) or length_changed($trains) then 
			   ...
			   par
				  ...
				  if contains(train_state($trains), NOTLOSTINTEGRITY) and contains(train_state($trains), NOTWAITINTEGRITYTIMER)  and contains(train_state($trains), NOTLENGTHCHANGED) then
	...
	macro rule r_Set_Vss_Unknown_Whitinma_When_Train_Disconnected($trains in Trains, $ttd in Ttd, $ma_trains in Prod(Vss_Units,Vss_Units), $trains_position in Vss_Units) =
			    ...
			    switch($ttd)
					case ttd_20:   //first free TTD
					     forall $vss in Vss with vss_rear_ttd($vss, $ttd) and ttd_states(vss_on_ttd($vss)) = TTD_OCCUPIED do						    	     
					...
					case ttd_30:  //first free TTD
					 	  forall $vss2 in Vss with vss_rear_ttd($vss2, $ttd) and ttd_states(vss_on_ttd($vss2)) = TTD_OCCUPIED do				     
...
// 4.3
	macro rule r_check_loss_integrity_situations($trains in Trains) =	
		  ...
		  par			    
			...
			forall $vss in Vss, $ttd in Ttd with $vss = vss_rear_train($trains) and ttd_states($ttd) = TTD_OCCUPIED and ttd_rear_train($trains) = $ttd do 
			  	if not integrityloss_propagation_timer(vss_rear_train($trains)) and train_on_ttd($ttd, $trains) then
...
// 3.5	Operation of trains treated as integer			
	macro rule r_Trains_integer ($trains in Trains) =
			...
			if trains_confirmed_integrity($trains) and train_position($trains) <= second(ma($trains))  and  rbc_conn($trains) and not trains_shadow($trains) then
			   par  
			     ...
			     if train_left_vss($trains) and not trains_shadow($trains) then 			                 
	                par				                         
	...
	macro rule r_Trains_reconnecting($trains in Trains, $vss in Vss) =
			...
			if not mute_timer($trains) and vss_states($vss) = UNKNOWN and train_inthis_vss($trains, $vss) and contains(train_state($trains), NOTCONNECTED) then 
			  par 					  		  			     
				 ...
				 if first(ma($trains)) = fourth(collect_trains_data($trains)) and  second(ma($trains)) = fifth(collect_trains_data($trains)) then
			       if (isDef(vss_advance_train($trains))) then			        
			          par			   
			              vss_states(vss_advance_train($trains)) := FREE
			              vss_prestates(vss_advance_train($trains)) := vss_states(vss_advance_train($trains))	             
			          endpar 
			       endif          			       					  
			     endif 	
			     forall $tr in Trains with $tr != $trains and train_rear_another_train($tr, $trains) do	     
			       if (trains_confirmed_integrity($trains)) and not(length_changed($trains)) and not train_inthis_vss($tr, vss_related_to_train_position(train_position($trains))) then 	
					 if (isDef(vss_rear_train($trains))) then			        	  
					   par			             
			              vss_states(vss_rear_train($trains)) := FREE
			              vss_prestates(vss_rear_train($trains)) := vss_states(vss_rear_train($trains)) 		            	  
			           endpar
...
default init initial_state:
	...
	function vss_states ($vss in Vss) = switch($vss)
											case vss_11: UNKNOWN
											case vss_12: UNKNOWN
											case vss_21: FREE
											case vss_22: FREE
											...
											case vss_33: FREE
										endswitch
	function vss_prestates ($vss in Vss) = switch($vss)
											case vss_11: UNKNOWN
											case vss_12: UNKNOWN
											case vss_21: FREE
											case vss_22: FREE
	...
	function ttd_states($ttd in Ttd) = switch($ttd)
											case ttd_10: TTD_OCCUPIED
											case ttd_20: TTD_FREE
											case ttd_30: TTD_FREE
									  endswitch
	...
	function trains_confirmed_integrity($trains in Trains) = false
	function confirmed_safe_rear_end($trains in Trains) = 0
	function shortened_ma($trains in Trains) = 0
	
	function collect_trains_data($trains in Trains) =  (10, 5, 0, 5, 40)
	function collect_trains_predata($trains in Trains) = (0, 0, 0, 0, 0)  
	function train_state($trains in Trains) =   [NOTLOSTINTEGRITY,  NOTLENGTHCHANGED, NOTCONNECTED, NOTSHADOWTRAIN, NOTINTEGRITYCONFIRMED, NOTWAITINTEGRITYTIMER]
	function train_prestate($trains in Trains) = [NOTLOSTINTEGRITY,  NOTLENGTHCHANGED, NOTCONNECTED, NOTSHADOWTRAIN, NOTINTEGRITYCONFIRMED, NOTWAITINTEGRITYTIMER]
	
	function mute_timer($trains in Trains) = true                 	
