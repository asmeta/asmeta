...
definitions:
	...
	function errorFORbpStatus = error(BP_ROTATION_DIR) or error(BP_NO_FLOW) or error(INIT_VP_UP) or error(INIT_AP_UP) or error(INIT_VP_LOW) or error(INIT_AP_LOW) or error(CONN_VP_UP) or error(CONN_VP_LOW) or error(CONN_AP_LOW) or error(FILL_BLOOD_VOL) or error(HEPARIN_DIR) or error(SAD_ERR) or error(ARTERIAL_BOLUS)
	...
	rule r_error_AP_low_BPflow_time ($err in AlarmErrorType, $errTime in Boolean, $errBPFlow in Boolean) =
		if error($err) and not(alarm($err)) then
			par
				...
				if not($errBPFlow) and not($errTime) then //is this correct?
					error($err) := false
				endif
			endpar
		endif
	...
	rule r_error_UF_rate =
		if error(UF_RATE) and not(alarm(UF_RATE)) then
			if error_UF_rate_resolved then
				error(UF_RATE) := false //MP4: it is already true
			endif
		endif
	...
	rule r_error_UF_dir =
		if error(UF_DIR) and not(alarm(UF_DIR)) then
			if error_UF_dir_resolved then
				error(UF_DIR) := false //MP4: it already true
			endif
		endif
	...
	rule r_end_connection =
		par
			...
			//signal_lamp := GREEN //MP4: it is already GREEN
	...
	//R18-R19 (already specified in HemodialysisRef1)
	...
	//R20 (modified w.r.t. the version in HemodialysisRef1)
	...
	//R23-R32 (modified w.r.t. the version in HemodialysisRef1)
	CTLSPEC ctlSpec_R23and32: ag((((phase = PREPARATION and prepPhase = TUBING_SYSTEM) or (phase = INITIATION and bp_status_der = START)) and (passed1Msec and currentSAD != PERMITTED and current_air_vol != PERMITTED and not(error(SAD_ERR)))) implies ax(error(SAD_ERR) and alarm(SAD_ERR)))
	LTLSPEC ltlSpec_R23and32: g((((phase = PREPARATION and prepPhase = TUBING_SYSTEM) or (phase = INITIATION and bp_status_der = START)) and (passed1Msec and currentSAD != PERMITTED and current_air_vol != PERMITTED and not(error(SAD_ERR)))) implies x(error(SAD_ERR) and alarm(SAD_ERR)))
