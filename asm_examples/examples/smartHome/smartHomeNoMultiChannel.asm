...
signature:
	...
	//Knowledge - signals
	dynamic controlled sgnHeaterOFF: Boolean
	dynamic controlled sgnHeaterFAIRLY_HOT: Boolean
	dynamic controlled sgnHeaterVERY_HOT: Boolean
	dynamic controlled sgnWaterHeaterON: Boolean
	dynamic controlled sgnWaterHeaterOFF: Boolean
	dynamic controlled sgnOpenWindow: Boolean
	dynamic controlled sgnCloseWindow: Boolean
...
definitions:
	...
	rule r_adaptHeaterMAPE_CH =
		par
			if sgnHeaterVERY_HOT then
				r_modifyHeaterStatus[VERY_HOT, sgnHeaterVERY_HOT]
			endif
			if sgnHeaterFAIRLY_HOT then
				r_modifyHeaterStatus[FAIRLY_HOT, sgnHeaterFAIRLY_HOT]
			endif
			if sgnHeaterOFF then
				r_modifyHeaterStatus[OFF, sgnHeaterOFF]
			endif
		endpar
	...
	rule r_adaptWindowMAPE_MD =
		if sgnCloseWindow then
			r_modifyWindowStatus[CLOSED, sgnCloseWindow]
		endif

	rule r_adaptWindowMAPE_AQ =
		par
			if sgnOpenWindow then
				r_modifyWindowStatus[OPEN, sgnOpenWindow]
			endif
			if sgnCloseWindow then
				r_modifyWindowStatus[CLOSED, sgnCloseWindow]
			endif
	...
	rule r_adaptWaterHeaterMAPE_MWH =
		if sgnWaterHeaterON then
			r_modifyWaterHeaterStatus[WE_ON, sgnWaterHeaterON]
		endif

	rule r_adaptWaterHeaterMAPE_ES =
		if sgnWaterHeaterOFF then
			r_modifyWaterHeaterStatus[WE_OFF, sgnWaterHeaterOFF]
		endif
	...
	rule r_checkRoomTempMAPE_CH =
		if roomTemp < 10 then
			sgnHeaterVERY_HOT := true // send signal to set the heater to VERY_HOT
		else
			if roomTemp < 18 then
				sgnHeaterFAIRLY_HOT := true // send signal to set the heater to FAIRLY_HOT
			else
				sgnHeaterOFF := true // send signal to turn the heater off
			endif
		endif
	...
	rule r_checkWindowAndHeaterMAPE_MD =
		if heaterStatus != OFF and windowStatus = OPEN then
			sgnCloseWindow := true // send signal to close the window
		endif

	//MAPE_AQ: do open window when air quality is bad
	rule r_checkAirQualityMAPE_AQ =
		if badAir then
			sgnOpenWindow := true // send signal to open the window
		else
			sgnCloseWindow := true // send signal to close the window
		endif
	...
	rule r_checkHotWaterMorningMAPE_MWH =
		if time = EARLY_MORN then
			sgnWaterHeaterON := true // send signal to turn the water heater on
		endif
		...
		if time = MORN_AFT then
			if heaterStatus = VERY_HOT and waterHeaterStatus = WE_ON then
				if esAdaptHeater then
					sgnHeaterFAIRLY_HOT := true // send signal to decrease the heater
				else
					sgnWaterHeaterOFF := true // send signal to turn the water heater off
				endif
			endif
		endif
	...
	LTLSPEC g((	(checkForAdaptation and execMAPE_CH and sgnHeaterOFF) or
				(checkForAdaptation and execMAPE_CH and sgnHeaterFAIRLY_HOT) or
				(checkForAdaptation and execMAPE_CH and sgnHeaterVERY_HOT) )
				implies
	...
	LTLSPEC g((checkForAdaptation and execMAPE_MD and sgnCloseWindow) implies o(checkForAdaptation and monMAPE_MD and heaterStatus != OFF and windowStatus = OPEN))
	LTLSPEC g(((checkForAdaptation and execMAPE_AQ and sgnOpenWindow) or (checkForAdaptation and execMAPE_AQ and sgnCloseWindow)) implies o((checkForAdaptation and monMAPE_AQ and badAir) or (checkForAdaptation and monMAPE_AQ and not(badAir))))
	LTLSPEC g((checkForAdaptation and execMAPE_MWH and sgnWaterHeaterON) implies o(checkForAdaptation and monMAPE_MWH and time = EARLY_MORN))
	LTLSPEC g((checkForAdaptation and execMAPE_ES and sgnHeaterFAIRLY_HOT) implies o(checkForAdaptation and monMAPE_ES and time = MORN_AFT and heaterStatus = VERY_HOT and waterHeaterStatus = WE_ON and esAdaptHeater))
	LTLSPEC g((checkForAdaptation and execMAPE_ES and sgnWaterHeaterOFF) implies o(checkForAdaptation and monMAPE_ES and time = MORN_AFT and heaterStatus = VERY_HOT and waterHeaterStatus = WE_ON and not(esAdaptHeater)))
...
default init s0:
	...
	//signals
	function sgnHeaterOFF = false
	function sgnHeaterFAIRLY_HOT = false
	function sgnHeaterVERY_HOT = false
	function sgnWaterHeaterON = false
	function sgnWaterHeaterOFF = false
	function sgnOpenWindow = false
	function sgnCloseWindow = false
