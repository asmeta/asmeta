asm GilbreathCardTrickSimpl

//Almost equal to the model GilbreathCardTrick.asm
//The model has been refactored with the macro rules
//r_incrementIndexOrSetUndef and r_riffleShuffle 
...
definitions:
	...
	rule r_incrementIndexOrSetUndef($f in Index, $limit in Index) =
		if $f < $limit then
			$f := $f + 1
		else
			$f := undef
	...
	rule r_riffleShuffle($initDeck in Index, $limit in Index) =
		par
			r_moveCard[$initDeck, free]
			r_shiftCards[free, $initDeck - 1]
			r_incrementIndexOrSetUndef[$initDeck, $limit]
		endpar
		...
		switch (step)
			...
			case SHUFFLE:
				...
				choose $pickFirst in Boolean with (isUndef(indexFirstDeck) implies not($pickFirst)) and
					...
					par
						r_incrementIndexOrSetUndef[free, 12]
						if $pickFirst then
							r_riffleShuffle[indexFirstDeck, cut]
						else
							par
								r_riffleShuffle[indexSecondDeck, 12]
								cut := cut + 1
								if(isDef(indexFirstDeck)) then
									r_incrementIndexOrSetUndef[indexFirstDeck, cut]
								endif
							endpar
						endif
					endpar
