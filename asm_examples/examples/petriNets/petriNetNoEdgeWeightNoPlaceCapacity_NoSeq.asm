asm petriNetNoEdgeWeightNoPlaceCapacity_NoSeq
...
definitions:
	...
	rule r_fire($t in Transition) =
		par
			forall $i in inputPlaces($t) with not(contains(outputPlaces($t), $i)) do
				tokens($i) := tokens($i) - 1
			forall $o in outputPlaces($t) with not(contains(inputPlaces($t), $o)) do
				tokens($o) := tokens($o) + 1
