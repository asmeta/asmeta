asm petriNetEdgeWeightNoPlaceCapacity_NoSeq_Matrix_v2
...
//just for the initial marking.
//This initial marking permits to have a real nondeterminism in the choice of the
//transition to fire.
//The nondeterministic choice is executed just in the first step (both t1 and t2
//can fire): the fire of both transitions produces the same marking.
//Then, in the following 6 steps, there is always just one transition that can fire.
...
default init s0:
	 //initial marking
 	function tokens($p in Place) = at({p1 -> 1, p2 -> 3, p3 -> 4, p4 -> 1}, $p)