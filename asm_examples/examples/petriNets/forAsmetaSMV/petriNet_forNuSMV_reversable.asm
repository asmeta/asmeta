asm petriNet_forNuSMV_reversable
...
//- the Petri net is different; this net has been designed in order to be reversable
//  the example is taken from the paper of Murata (pag. 9 (or 549), Fig.17 (e))

//Definition of reversibility
//A Petri net (N,M0) is said to be reversible if, for each marking M \in R(M0),
//M0 is reachable from M. Thus, in a reversible net one can always get back to
//the initial marking or state.
...
definitions:
	...
	function inArcWeight($p in Place, $t in Transition) =
		...
		switch($p)
			...
			case p4: if($t = t3) then 1 else 0 endif
 	...
 	function outArcWeight($t in Transition, $p in Place) =
		...
		switch($p)
			...
			case p3: if($t = t1) then 1 else 0 endif
			case p4: if($t = t4) then 1 else 0 endif
			case p5: if($t = t3) then 1 else 0 endif
		endswitch
	...
	function capacity($p in Place) = if($p = p1) then 10 else 1 endif
	...
	//Is it reversable? Yes, it's reversable.
	...
	//It is 9-bounded? No, it's not.
	CTLSPEC (forall $p in Place with ag(tokens($p) <= 9))

	invariant over tokens: (forall $p in Place with tokens($p) >= 0 and tokens($p) <= capacity($p))
...
default init s0:
	 //initial marking
 	function tokens($p in Place) = at({p1 -> 0, p2 -> 0, p3 -> 1, p4 -> 1, p5 -> 0}, $p) 
