...
definitions:
    ...
    main rule r_Main =
        if isDef(chooseVar0) then
            par
                tokens(p3) := plus(tokens(p3),incidenceMatrix(p3,chooseVar0))
                tokens(p4) := plus(tokens(p4),incidenceMatrix(p4,chooseVar0))
                tokens(p2) := plus(tokens(p2),incidenceMatrix(p2,chooseVar0))
                tokens(p1) := plus(tokens(p1),incidenceMatrix(p1,chooseVar0))
            endpar
        endif
