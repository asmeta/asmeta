...
definitions:
	...
	rule r_healthMonitoring =
		par
			if openDoorsElectroValve and not(doorsAllNotClosed) and timeout then
				anomaly := true
			endif
			if openDoorsElectroValve and not(doorsOpen) and timeout then
				anomaly := true
			endif
			if closeDoorsElectroValve and not(doorsAllNotOpen) and timeout then
				anomaly := true
			endif
			...
			if retractGearsElectroValve and not(gearsAllNotExtended) and timeout then
				anomaly := true
			endif
			if retractGearsElectroValve and not(gearsRetracted) and timeout then
				anomaly := true
			endif
			if extendGearsElectroValve and not(gearsAllNotRetracted) and timeout then
				anomaly := true
			endif
	...
	//R61
	CTLSPEC ag((openDoorsElectroValve and not(doorsAllNotClosed) and timeout) implies ax(ag(anomaly)))
	//R62
	CTLSPEC ag((closeDoorsElectroValve and not(doorsAllNotOpen) and timeout) implies ax(ag(anomaly)))
	//R63
	CTLSPEC ag((retractGearsElectroValve and not(gearsAllNotExtended) and timeout) implies ax(ag(anomaly)))
	//R64
	CTLSPEC ag((extendGearsElectroValve and not(gearsAllNotRetracted) and timeout) implies ax(ag(anomaly)))
