...
definitions:
	...
	rule r_philoRule($p in Philosopher) =
		if(cherriesInPlate($p) > 0) then
			...
			if(cherriesInPlate(leftNeighbour($p)) > 0) then
				...
				if(cherriesInPlate(rightNeighbour($p)) > 0) then
					par
						cherriesInPlate(rightNeighbour($p)) := cherriesInPlate(rightNeighbour($p)) - 1
						cherriesInPlate($p) := cherriesInPlate($p) + 1
					endpar
				endif
			endif
	...
	CTLSPEC ag(cherriesInPlate(philo1) <= 3)
	CTLSPEC ag(cherriesInPlate(philo2) <= 3)
	CTLSPEC ag(cherriesInPlate(philo3) <= 3)
	CTLSPEC ag(cherriesInPlate(philo4) <= 3)
	CTLSPEC ag(cherriesInPlate(philo5) <= 3)
