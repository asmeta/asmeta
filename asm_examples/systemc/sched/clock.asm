...
module clock
	...
	import common
	import chanbool
	export Clock, r_initClock, clockOutput, defaultEvent, posEdgeEvent, negEdgeEvent
...
definitions:
	...
	macro rule r_runClock =
		let ($p = current_exec,
			...
			endseq else
				let ($value = read(chan($clock))) in seq
	...
	$posEdgeFirst in Boolean) =
		extend ChanBoolean with $chan do 
