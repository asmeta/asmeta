...
definitions:
	...
	//Contengono le precedenti ed altri casi.
	//Reintroducono AG ma inserendo il controllo sulla presenza di soldi sul conto.
	CTLSPEC ag((inbox(AA, REQ, BB, 5n, 1n) and (balance(AA)>=5n)) implies eu(inbox(AA, REQ, BB, 5n, 1n), inbox(BB, VAL, AA, 5n, 1n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 5n, 1n) and (balance(BB)>=5n)) implies eu(inbox(BB, REQ, AA, 5n, 1n), inbox(AA, VAL, BB, 5n, 1n)))
	CTLSPEC ag((inbox(AA, REQ, BB, 5n, 2n) and (balance(AA)>=5n)) implies eu(inbox(AA, REQ, BB, 5n, 2n), inbox(BB, VAL, AA, 5n, 2n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 5n, 2n) and (balance(BB)>=5n)) implies eu(inbox(BB, REQ, AA, 5n, 2n), inbox(AA, VAL, BB, 5n, 2n)))
	CTLSPEC ag((inbox(AA, REQ, BB, 10n, 1n) and (balance(AA)>=10n)) implies eu(inbox(AA, REQ, BB, 10n, 1n), inbox(BB, VAL, AA, 10n, 1n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 10n, 1n) and (balance(BB)>=10n)) implies eu(inbox(BB, REQ, AA, 10n, 1n), inbox(AA, VAL, BB, 10n, 1n)))
	CTLSPEC ag((inbox(AA, REQ, BB, 10n, 2n) and (balance(AA)>=10n)) implies eu(inbox(AA, REQ, BB, 10n, 2n), inbox(BB, VAL, AA, 10n, 2n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 10n, 2n) and (balance(BB)>=10n)) implies eu(inbox(BB, REQ, AA, 10n, 2n), inbox(AA, VAL, BB, 10n, 2n)))
	...
	//dell'altra scheda
	CTLSPEC ag(inbox(AA, VAL, BB, 0n, 1n) implies eu(inbox(AA, VAL, BB, 0n, 1n), inbox(BB, ACK, AA, 0n, 1n)))
	CTLSPEC ag(inbox(BB, VAL, AA, 0n, 1n) implies eu(inbox(BB, VAL, AA, 0n, 1n), inbox(AA, ACK, BB, 0n, 1n)))
	CTLSPEC ag(inbox(AA, VAL, BB, 0n, 2n) implies eu(inbox(AA, VAL, BB, 0n, 2n), inbox(BB, ACK, AA, 0n, 2n)))
	CTLSPEC ag(inbox(BB, VAL, AA, 0n, 2n) implies eu(inbox(BB, VAL, AA, 0n, 2n), inbox(AA, ACK, BB, 0n, 2n)))
	CTLSPEC ag(inbox(AA, VAL, BB, 5n, 1n) implies eu(inbox(AA, VAL, BB, 5n, 1n), inbox(BB, ACK, AA, 5n, 1n)))
	CTLSPEC ag(inbox(BB, VAL, AA, 5n, 1n) implies eu(inbox(BB, VAL, AA, 5n, 1n), inbox(AA, ACK, BB, 5n, 1n)))
	CTLSPEC ag(inbox(AA, VAL, BB, 5n, 2n) implies eu(inbox(AA, VAL, BB, 5n, 2n), inbox(BB, ACK, AA, 5n, 2n)))
	CTLSPEC ag(inbox(BB, VAL, AA, 5n, 2n) implies eu(inbox(BB, VAL, AA, 5n, 2n), inbox(AA, ACK, BB, 5n, 2n)))
	CTLSPEC ag(inbox(AA, VAL, BB, 10n, 1n) implies eu(inbox(AA, VAL, BB, 10n, 1n), inbox(BB, ACK, AA, 10n, 1n)))
	CTLSPEC ag(inbox(BB, VAL, AA, 10n, 1n) implies eu(inbox(BB, VAL, AA, 10n, 1n), inbox(AA, ACK, BB, 10n, 1n)))
	CTLSPEC ag(inbox(AA, VAL, BB, 10n, 2n) implies eu(inbox(AA, VAL, BB, 10n, 2n), inbox(BB, ACK, AA, 10n, 2n)))
	CTLSPEC ag(inbox(BB, VAL, AA, 10n, 2n) implies eu(inbox(BB, VAL, AA, 10n, 2n), inbox(AA, ACK, BB, 10n, 2n)))
