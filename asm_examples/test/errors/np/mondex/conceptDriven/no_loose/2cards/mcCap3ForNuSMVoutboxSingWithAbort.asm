...
//non c'e' il metodo loose messages
...
definitions:
	...
	function outBoxEqualInbox =
		if((forall $card1 in Name, $card2 in Name, $value in MoneyDomain,
			$tid in TidDomain with( (inbox($card1, REQ, $card2, $value, $tid) and not(exLogTo($card2,$card1,$value,$tid))) implies
						(not(outboxIsNone($card2)) and
						outboxMessage($card2) = REQ and
						outboxName($card2) = $card1 and
						outboxMoney($card2) = $value and
						outboxTid($card2) = $tid) )) and
			(forall $card12 in Name, $card22 in Name, $value2 in MoneyDomain,
			$tid2 in TidDomain with( (inbox($card12, VAL, $card22, $value2, $tid2) and not(exLogFrom($card22,$card12,$value2,$tid2))) implies
						(not(outboxIsNone($card22)) and
						outboxMessage($card22) = VAL and
						outboxName($card22) = $card12 and
						outboxMoney($card22) = $value2 and
						outboxTid($card22) = $tid2) ))
	...
	//non e' possibile che una carta invii il messaggio di ACK ed abbia,
	// allo stesso tempo, la exLogTo della transizione
	function correctExLogTo = 
		if(forall $card1 in Name, $card2 in Name, $value in MoneyDomain,
			$tid in TidDomain with not(inbox($card2, ACK, $card1, $value, $tid) and exLogTo($card1, $card2, $value, $tid))) then
			true
		else
			false
		endif
	...
	//introducendo la regola r_abort il problema si risolve perche' si puo'
	//interrompere una delle due transizioni
	...
	//perche' in alcuni casi la transizioni potrebbero non concludersi perche' non ci sono soldi
	//sul conto
	...
	CTLSPEC ag((inbox(AA, REQ, BB, 5n, 1n) and ef( balance(AA) >= 5n)) implies ef(inbox(BB, VAL, AA, 5n, 1n) or exLogTo(BB, AA, 5n, 1n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 5n, 1n) and ef( balance(BB) >= 5n)) implies ef(inbox(AA, VAL, BB, 5n, 1n) or exLogTo(AA, BB, 5n, 1n)))
	CTLSPEC ag((inbox(AA, REQ, BB, 5n, 2n) and ef( balance(AA) >= 5n)) implies ef(inbox(BB, VAL, AA, 5n, 2n) or exLogTo(BB, AA, 5n, 2n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 5n, 2n) and ef( balance(BB) >= 5n)) implies ef(inbox(AA, VAL, BB, 5n, 2n) or exLogTo(AA, BB, 5n, 2n)))
	CTLSPEC ag((inbox(AA, REQ, BB, 10n, 1n) and ef( balance(AA) >= 10n)) implies ef(inbox(BB, VAL, AA, 10n, 1n) or exLogTo(BB, AA, 10n, 1n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 10n, 1n) and ef( balance(BB) >= 10n)) implies ef(inbox(AA, VAL, BB, 10n, 1n) or exLogTo(AA, BB, 10n, 1n)))
	CTLSPEC ag((inbox(AA, REQ, BB, 10n, 2n) and ef( balance(AA) >= 10n)) implies ef(inbox(BB, VAL, AA, 10n, 2n) or exLogTo(BB, AA, 10n, 2n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 10n, 2n) and ef( balance(BB) >= 10n)) implies ef(inbox(AA, VAL, BB, 10n, 2n) or exLogTo(AA, BB, 10n, 2n)))
	//Le 8 precedenti posso essere eseguite anche senza il controllo sul conto.
	//In caso di mancanza di soldi sul conto la situazione puo' essere risolta dalla r_abort.
	CTLSPEC ag((inbox(AA, REQ, BB, 5n, 1n)) implies ef(inbox(BB, VAL, AA, 5n, 1n) or exLogTo(BB, AA, 5n, 1n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 5n, 1n)) implies ef(inbox(AA, VAL, BB, 5n, 1n) or exLogTo(AA, BB, 5n, 1n)))
	CTLSPEC ag((inbox(AA, REQ, BB, 5n, 2n)) implies ef(inbox(BB, VAL, AA, 5n, 2n) or exLogTo(BB, AA, 5n, 2n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 5n, 2n)) implies ef(inbox(AA, VAL, BB, 5n, 2n) or exLogTo(AA, BB, 5n, 2n)))
	CTLSPEC ag((inbox(AA, REQ, BB, 10n, 1n)) implies ef(inbox(BB, VAL, AA, 10n, 1n) or exLogTo(BB, AA, 10n, 1n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 10n, 1n)) implies ef(inbox(AA, VAL, BB, 10n, 1n) or exLogTo(AA, BB, 10n, 1n)))
	CTLSPEC ag((inbox(AA, REQ, BB, 10n, 2n)) implies ef(inbox(BB, VAL, AA, 10n, 2n) or exLogTo(BB, AA, 10n, 2n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 10n, 2n)) implies ef(inbox(AA, VAL, BB, 10n, 2n) or exLogTo(AA, BB, 10n, 2n)))
	...
	//Bisogna tenere presente che, anche in presenza di soldi sul conto, la transizione
	//potrebbe non concludersi perche' viene eseguita una r_abort
	CTLSPEC ag((inbox(AA, REQ, BB, 5n, 1n) and ef(balance(AA)>=5n)) implies ef(ag(not(inbox(AA, REQ, BB, 5n, 1n)) or exLogTo(BB, AA, 5n, 1n))))
	CTLSPEC ag((inbox(BB, REQ, AA, 5n, 1n) and ef(balance(BB)>=5n)) implies ef(ag(not(inbox(BB, REQ, AA, 5n, 1n)) or exLogTo(AA, BB, 5n, 1n))))
	CTLSPEC ag((inbox(AA, REQ, BB, 5n, 2n) and ef(balance(AA)>=5n)) implies ef(ag(not(inbox(AA, REQ, BB, 5n, 2n)) or exLogTo(BB, AA, 5n, 2n))))
	CTLSPEC ag((inbox(BB, REQ, AA, 5n, 2n) and ef(balance(BB)>=5n)) implies ef(ag(not(inbox(BB, REQ, AA, 5n, 2n)) or exLogTo(AA, BB, 5n, 2n))))
	CTLSPEC ag((inbox(AA, REQ, BB, 10n, 1n) and ef(balance(AA)>=10n)) implies ef(ag(not(inbox(AA, REQ, BB, 10n, 1n)) or exLogTo(BB, AA, 10n, 1n))))
	CTLSPEC ag((inbox(BB, REQ, AA, 10n, 1n) and ef(balance(BB)>=10n)) implies ef(ag(not(inbox(BB, REQ, AA, 10n, 1n)) or exLogTo(AA, BB, 10n, 1n))))
	CTLSPEC ag((inbox(AA, REQ, BB, 10n, 2n) and ef(balance(AA)>=10n)) implies ef(ag(not(inbox(AA, REQ, BB, 10n, 2n)) or exLogTo(BB, AA, 10n, 2n))))
	CTLSPEC ag((inbox(BB, REQ, AA, 10n, 2n) and ef(balance(BB)>=10n)) implies ef(ag(not(inbox(BB, REQ, AA, 10n, 2n)) or exLogTo(AA, BB, 10n, 2n))))
	
	//se una carta e' in val, l'altra carta prima o poi sara' in ack o e' stato memorizzato un exLogTo
	CTLSPEC ag(inbox(AA, VAL, BB, 0n, 1n) implies ef(inbox(BB, ACK, AA, 0n, 1n) or exLogTo(AA, BB, 0n, 1n)))
	CTLSPEC ag(inbox(BB, VAL, AA, 0n, 1n) implies ef(inbox(AA, ACK, BB, 0n, 1n) or exLogTo(BB, AA, 0n, 1n)))
	CTLSPEC ag(inbox(AA, VAL, BB, 0n, 2n) implies ef(inbox(BB, ACK, AA, 0n, 2n) or exLogTo(AA, BB, 0n, 2n)))
	CTLSPEC ag(inbox(BB, VAL, AA, 0n, 2n) implies ef(inbox(AA, ACK, BB, 0n, 2n) or exLogTo(BB, AA, 0n, 2n)))
	CTLSPEC ag(inbox(AA, VAL, BB, 5n, 1n) implies ef(inbox(BB, ACK, AA, 5n, 1n) or exLogTo(AA, BB, 5n, 1n)))
	CTLSPEC ag(inbox(BB, VAL, AA, 5n, 1n) implies ef(inbox(AA, ACK, BB, 5n, 1n) or exLogTo(BB, AA, 5n, 1n)))
	CTLSPEC ag(inbox(AA, VAL, BB, 5n, 2n) implies ef(inbox(BB, ACK, AA, 5n, 2n) or exLogTo(AA, BB, 5n, 2n)))
	CTLSPEC ag(inbox(BB, VAL, AA, 5n, 2n) implies ef(inbox(AA, ACK, BB, 5n, 2n) or exLogTo(BB, AA, 5n, 2n)))
	CTLSPEC ag(inbox(AA, VAL, BB, 10n, 1n) implies ef(inbox(BB, ACK, AA, 10n, 1n) or exLogTo(AA, BB, 10n, 1n)))
	CTLSPEC ag(inbox(BB, VAL, AA, 10n, 1n) implies ef(inbox(AA, ACK, BB, 10n, 1n) or exLogTo(BB, AA, 10n, 1n)))
	CTLSPEC ag(inbox(AA, VAL, BB, 10n, 2n) implies ef(inbox(BB, ACK, AA, 10n, 2n) or exLogTo(AA, BB, 10n, 2n)))
	CTLSPEC ag(inbox(BB, VAL, AA, 10n, 2n) implies ef(inbox(AA, ACK, BB, 10n, 2n) or exLogTo(BB, AA, 10n, 2n)))
	...
	CTLSPEC ag(inbox(AA, VAL, BB, 0n, 1n) implies ef(ag(not(inbox(AA, VAL, BB, 0n, 1n)) or exLogTo(AA,BB,0n,1n))))
	CTLSPEC ag(inbox(BB, VAL, AA, 0n, 1n) implies ef(ag(not(inbox(BB, VAL, AA, 0n, 1n)) or exLogTo(BB,AA,0n,1n))))
	CTLSPEC ag(inbox(AA, VAL, BB, 0n, 2n) implies ef(ag(not(inbox(AA, VAL, BB, 0n, 2n)) or exLogTo(AA,BB,0n,2n))))
	CTLSPEC ag(inbox(BB, VAL, AA, 0n, 2n) implies ef(ag(not(inbox(BB, VAL, AA, 0n, 2n)) or exLogTo(BB,AA,0n,2n))))
	CTLSPEC ag(inbox(AA, VAL, BB, 5n, 1n) implies ef(ag(not(inbox(AA, VAL, BB, 5n, 1n)) or exLogTo(AA,BB,5n,1n))))
	CTLSPEC ag(inbox(BB, VAL, AA, 5n, 1n) implies ef(ag(not(inbox(BB, VAL, AA, 5n, 1n)) or exLogTo(BB,AA,5n,1n))))
	CTLSPEC ag(inbox(AA, VAL, BB, 5n, 2n) implies ef(ag(not(inbox(AA, VAL, BB, 5n, 2n)) or exLogTo(AA,BB,5n,2n))))
	CTLSPEC ag(inbox(BB, VAL, AA, 5n, 2n) implies ef(ag(not(inbox(BB, VAL, AA, 5n, 2n)) or exLogTo(BB,AA,5n,2n))))
	CTLSPEC ag(inbox(AA, VAL, BB, 10n, 1n) implies ef(ag(not(inbox(AA, VAL, BB, 10n, 1n)) or exLogTo(AA,BB,10n,1n))))
	CTLSPEC ag(inbox(BB, VAL, AA, 10n, 1n) implies ef(ag(not(inbox(BB, VAL, AA, 10n, 1n)) or exLogTo(BB,AA,10n,1n))))
	CTLSPEC ag(inbox(AA, VAL, BB, 10n, 2n) implies ef(ag(not(inbox(AA, VAL, BB, 10n, 2n)) or exLogTo(AA,BB,10n,2n))))
	CTLSPEC ag(inbox(BB, VAL, AA, 10n, 2n) implies ef(ag(not(inbox(BB, VAL, AA, 10n, 2n)) or exLogTo(BB,AA,10n,2n))))
	
	//se una carta e' in ACK con la regola r_ack ne uscira. Quello stato (tripla valore-tid-ACK)
	//non si potra piu' presentare. Se rimane selezionata viene creata una exLogTo
	CTLSPEC ag(inbox(AA, ACK, BB, 0n, 1n) implies ef(ag(not(inbox(AA, ACK, BB, 0n, 1n)) or exLogFrom(AA,BB,0n,1n))))
	CTLSPEC ag(inbox(BB, ACK, AA, 0n, 1n) implies ef(ag(not(inbox(BB, ACK, AA, 0n, 1n)) or exLogFrom(BB,AA,0n,1n))))
	CTLSPEC ag(inbox(AA, ACK, BB, 0n, 2n) implies ef(ag(not(inbox(AA, ACK, BB, 0n, 2n)) or exLogFrom(AA,BB,0n,2n))))
	CTLSPEC ag(inbox(BB, ACK, AA, 0n, 2n) implies ef(ag(not(inbox(BB, ACK, AA, 0n, 2n)) or exLogFrom(BB,AA,0n,2n))))
	CTLSPEC ag(inbox(AA, ACK, BB, 5n, 1n) implies ef(ag(not(inbox(AA, ACK, BB, 5n, 1n)) or exLogFrom(AA,BB,5n,1n))))
	CTLSPEC ag(inbox(BB, ACK, AA, 5n, 1n) implies ef(ag(not(inbox(BB, ACK, AA, 5n, 1n)) or exLogFrom(BB,AA,5n,1n))))
	CTLSPEC ag(inbox(AA, ACK, BB, 5n, 2n) implies ef(ag(not(inbox(AA, ACK, BB, 5n, 2n)) or exLogFrom(AA,BB,5n,2n))))
	CTLSPEC ag(inbox(BB, ACK, AA, 5n, 2n) implies ef(ag(not(inbox(BB, ACK, AA, 5n, 2n)) or exLogFrom(BB,AA,5n,2n))))
	CTLSPEC ag(inbox(AA, ACK, BB, 10n, 1n) implies ef(ag(not(inbox(AA, ACK, BB, 10n, 1n)) or exLogFrom(AA,BB,10n,1n))))
	CTLSPEC ag(inbox(BB, ACK, AA, 10n, 1n) implies ef(ag(not(inbox(BB, ACK, AA, 10n, 1n)) or exLogFrom(BB,AA,10n,1n))))
	CTLSPEC ag(inbox(AA, ACK, BB, 10n, 2n) implies ef(ag(not(inbox(AA, ACK, BB, 10n, 2n)) or exLogFrom(AA,BB,10n,2n))))
	CTLSPEC ag(inbox(BB, ACK, AA, 10n, 2n) implies ef(ag(not(inbox(BB, ACK, AA, 10n, 2n)) or exLogFrom(BB,AA,10n,2n))))
	
	//se una carta ha ricevuto la REQ, prima o poi dovrebbe ricevere anche l'ACK. Oppure viene creato un exLogTo
	//Dovrebbe valere per proprieta' transitiva dai gruppi di regole sopra, ma non si sa mai...
	CTLSPEC ag(inbox(AA, REQ, BB, 0n, 1n) implies ef(inbox(AA, ACK, BB, 0n, 1n) or exLogTo(BB,AA,0n,1n)))
	CTLSPEC ag(inbox(BB, REQ, AA, 0n, 1n) implies ef(inbox(BB, ACK, AA, 0n, 1n) or exLogTo(AA,BB,0n,1n)))
	CTLSPEC ag(inbox(AA, REQ, BB, 0n, 2n) implies ef(inbox(AA, ACK, BB, 0n, 2n) or exLogTo(BB,AA,0n,2n)))
	CTLSPEC ag(inbox(BB, REQ, AA, 0n, 2n) implies ef(inbox(BB, ACK, AA, 0n, 2n) or exLogTo(AA,BB,0n,2n)))
	//Per i prossimi bisogna essere meno restrittivi (non si puo' usare AG): bisogna usare EF
	//perche' in alcuni casi la transizioni potrebbero non concludersi perche' non ci sono soldi
	//sul conto
	...
	//Bisogna tenere presente che, anche in presenza di soldi sul conto, la transizione
	//potrebbe non concludersi perche' viene eseguita una r_abort
	CTLSPEC ag((inbox(AA, REQ, BB, 5n, 1n) and ef(balance(AA)>=5n)) implies ef(inbox(AA, ACK, BB, 5n, 1n) or exLogTo(BB,AA,5n,1n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 5n, 1n) and ef(balance(BB)>=5n)) implies ef(inbox(BB, ACK, AA, 5n, 1n) or exLogTo(AA,BB,5n,1n)))
	CTLSPEC ag((inbox(AA, REQ, BB, 5n, 2n) and ef(balance(AA)>=5n)) implies ef(inbox(AA, ACK, BB, 5n, 2n) or exLogTo(BB,AA,5n,2n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 5n, 2n) and ef(balance(BB)>=5n)) implies ef(inbox(BB, ACK, AA, 5n, 2n) or exLogTo(AA,BB,5n,2n)))
	CTLSPEC ag((inbox(AA, REQ, BB, 10n, 1n) and ef(balance(AA)>=10n)) implies ef(inbox(AA, ACK, BB, 10n, 1n) or exLogTo(BB,AA,10n,1n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 10n, 1n) and ef(balance(BB)>=10n)) implies ef(inbox(BB, ACK, AA, 10n, 1n) or exLogTo(AA,BB,10n,1n)))
	CTLSPEC ag((inbox(AA, REQ, BB, 10n, 2n) and ef(balance(AA)>=10n)) implies ef(inbox(AA, ACK, BB, 10n, 2n) or exLogTo(BB,AA,10n,2n)))
	CTLSPEC ag((inbox(BB, REQ, AA, 10n, 2n) and ef(balance(BB)>=10n)) implies ef(inbox(BB, ACK, AA, 10n, 2n) or exLogTo(AA,BB,10n,2n)))
		
	//se un messaggio e' nella inbox di $card1 e non c'e' stato abort, deve essere anche nella outbox
	//di $card2 che lo ha inviato. Vale solo per i messaggi di REQ e VAL
	CTLSPEC ag(outBoxEqualInbox)
	//la stessa proprieta' scomposta 
	CTLSPEC ag((inbox(AA, REQ, BB, 0n, 1n) and not(exLogTo(BB,AA,0n,1n))) implies (not(outboxIsNone(BB)) and outboxMessage(BB) = REQ and outboxName(BB) = AA and outboxMoney(BB) = 0n and outboxTid(BB) = 1n))
	CTLSPEC ag((inbox(BB, REQ, AA, 0n, 1n) and not(exLogTo(AA,BB,0n,1n))) implies (not(outboxIsNone(AA)) and outboxMessage(AA) = REQ and outboxName(AA) = BB and outboxMoney(AA) = 0n and outboxTid(AA) = 1n))
	CTLSPEC ag((inbox(AA, REQ, BB, 5n, 1n) and not(exLogTo(BB,AA,5n,1n))) implies (not(outboxIsNone(BB)) and outboxMessage(BB) = REQ and outboxName(BB) = AA and outboxMoney(BB) = 5n and outboxTid(BB) = 1n))
	CTLSPEC ag((inbox(BB, REQ, AA, 5n, 1n) and not(exLogTo(AA,BB,5n,1n))) implies (not(outboxIsNone(AA)) and outboxMessage(AA) = REQ and outboxName(AA) = BB and outboxMoney(AA) = 5n and outboxTid(AA) = 1n))
	CTLSPEC ag((inbox(AA, REQ, BB, 10n, 1n) and not(exLogTo(BB,AA,10n,1n))) implies (not(outboxIsNone(BB)) and outboxMessage(BB) = REQ and outboxName(BB) = AA and outboxMoney(BB) = 10n and outboxTid(BB) = 1n))
	CTLSPEC ag((inbox(BB, REQ, AA, 10n, 1n) and not(exLogTo(AA,BB,10n,1n))) implies (not(outboxIsNone(AA)) and outboxMessage(AA) = REQ and outboxName(AA) = BB and outboxMoney(AA) = 10n and outboxTid(AA) = 1n))
	CTLSPEC ag((inbox(AA, REQ, BB, 0n, 2n) and not(exLogTo(BB,AA,0n,2n))) implies (not(outboxIsNone(BB)) and outboxMessage(BB) = REQ and outboxName(BB) = AA and outboxMoney(BB) = 0n and outboxTid(BB) = 2n))
	CTLSPEC ag((inbox(BB, REQ, AA, 0n, 2n) and not(exLogTo(AA,BB,0n,2n))) implies (not(outboxIsNone(AA)) and outboxMessage(AA) = REQ and outboxName(AA) = BB and outboxMoney(AA) = 0n and outboxTid(AA) = 2n))
	CTLSPEC ag((inbox(AA, REQ, BB, 5n, 2n) and not(exLogTo(BB,AA,5n,2n))) implies (not(outboxIsNone(BB)) and outboxMessage(BB) = REQ and outboxName(BB) = AA and outboxMoney(BB) = 5n and outboxTid(BB) = 2n))
	CTLSPEC ag((inbox(BB, REQ, AA, 5n, 2n) and not(exLogTo(AA,BB,5n,2n))) implies (not(outboxIsNone(AA)) and outboxMessage(AA) = REQ and outboxName(AA) = BB and outboxMoney(AA) = 5n and outboxTid(AA) = 2n))
	CTLSPEC ag((inbox(AA, REQ, BB, 10n, 2n) and not(exLogTo(BB,AA,10n,2n))) implies (not(outboxIsNone(BB)) and outboxMessage(BB) = REQ and outboxName(BB) = AA and outboxMoney(BB) = 10n and outboxTid(BB) = 2n))
	CTLSPEC ag((inbox(BB, REQ, AA, 10n, 2n) and not(exLogTo(AA,BB,10n,2n))) implies (not(outboxIsNone(AA)) and outboxMessage(AA) = REQ and outboxName(AA) = BB and outboxMoney(AA) = 10n and outboxTid(AA) = 2n))
	CTLSPEC ag((inbox(AA, VAL, BB, 0n, 1n) and not(exLogFrom(BB,AA,0n,1n))) implies (not(outboxIsNone(BB)) and outboxMessage(BB) = VAL and outboxName(BB) = AA and outboxMoney(BB) = 0n and outboxTid(BB) = 1n))
	CTLSPEC ag((inbox(BB, VAL, AA, 0n, 1n) and not(exLogFrom(AA,BB,0n,1n))) implies (not(outboxIsNone(AA)) and outboxMessage(AA) = VAL and outboxName(AA) = BB and outboxMoney(AA) = 0n and outboxTid(AA) = 1n))
	CTLSPEC ag((inbox(AA, VAL, BB, 5n, 1n) and not(exLogFrom(BB,AA,5n,1n))) implies (not(outboxIsNone(BB)) and outboxMessage(BB) = VAL and outboxName(BB) = AA and outboxMoney(BB) = 5n and outboxTid(BB) = 1n))
	CTLSPEC ag((inbox(BB, VAL, AA, 5n, 1n) and not(exLogFrom(AA,BB,5n,1n))) implies (not(outboxIsNone(AA)) and outboxMessage(AA) = VAL and outboxName(AA) = BB and outboxMoney(AA) = 5n and outboxTid(AA) = 1n))
	CTLSPEC ag((inbox(AA, VAL, BB, 10n, 1n) and not(exLogFrom(BB,AA,10n,1n))) implies (not(outboxIsNone(BB)) and outboxMessage(BB) = VAL and outboxName(BB) = AA and outboxMoney(BB) = 10n and outboxTid(BB) = 1n))
	CTLSPEC ag((inbox(BB, VAL, AA, 10n, 1n) and not(exLogFrom(AA,BB,10n,1n))) implies (not(outboxIsNone(AA)) and outboxMessage(AA) = VAL and outboxName(AA) = BB and outboxMoney(AA) = 10n and outboxTid(AA) = 1n))
	CTLSPEC ag((inbox(AA, VAL, BB, 0n, 2n) and not(exLogFrom(BB,AA,0n,2n))) implies (not(outboxIsNone(BB)) and outboxMessage(BB) = VAL and outboxName(BB) = AA and outboxMoney(BB) = 0n and outboxTid(BB) = 2n))
	CTLSPEC ag((inbox(BB, VAL, AA, 0n, 2n) and not(exLogFrom(AA,BB,0n,2n))) implies (not(outboxIsNone(AA)) and outboxMessage(AA) = VAL and outboxName(AA) = BB and outboxMoney(AA) = 0n and outboxTid(AA) = 2n))
	CTLSPEC ag((inbox(AA, VAL, BB, 5n, 2n) and not(exLogFrom(BB,AA,5n,2n))) implies (not(outboxIsNone(BB)) and outboxMessage(BB) = VAL and outboxName(BB) = AA and outboxMoney(BB) = 5n and outboxTid(BB) = 2n))
	CTLSPEC ag((inbox(BB, VAL, AA, 5n, 2n) and not(exLogFrom(AA,BB,5n,2n))) implies (not(outboxIsNone(AA)) and outboxMessage(AA) = VAL and outboxName(AA) = BB and outboxMoney(AA) = 5n and outboxTid(AA) = 2n))
	CTLSPEC ag((inbox(AA, VAL, BB, 10n, 2n) and not(exLogFrom(BB,AA,10n,2n))) implies (not(outboxIsNone(BB)) and outboxMessage(BB) = VAL and outboxName(BB) = AA and outboxMoney(BB) = 10n and outboxTid(BB) = 2n))
	CTLSPEC ag((inbox(BB, VAL, AA, 10n, 2n) and not(exLogFrom(AA,BB,10n,2n))) implies (not(outboxIsNone(AA)) and outboxMessage(AA) = VAL and outboxName(AA) = BB and outboxMoney(AA) = 10n and outboxTid(AA) = 2n))	
	...
	//tutte false: danno un contro-esempio in cui si presenta il messaggio nella exLogTo (o exLogFrom)
	//quindi esiste per ognuna uno stato iniziale che le permette di presentarsi
	/*CTLSPEC not(ef(exLogTo(AA, BB, 0n, 1n)))
	CTLSPEC not(ef(exLogTo(BB, AA, 0n, 1n)))
	CTLSPEC not(ef(exLogTo(AA, BB, 5n, 1n)))
	CTLSPEC not(ef(exLogTo(BB, AA, 5n, 1n)))
	CTLSPEC not(ef(exLogTo(AA, BB, 10n, 1n)))
	CTLSPEC not(ef(exLogTo(BB, AA, 10n, 1n)))
	CTLSPEC not(ef(exLogTo(AA, BB, 0n, 2n)))
	CTLSPEC not(ef(exLogTo(BB, AA, 0n, 2n)))
	CTLSPEC not(ef(exLogTo(AA, BB, 5n, 2n)))
	CTLSPEC not(ef(exLogTo(BB, AA, 5n, 2n)))
	CTLSPEC not(ef(exLogTo(AA, BB, 10n, 2n)))
	CTLSPEC not(ef(exLogTo(BB, AA, 10n, 2n)))
	CTLSPEC not(ef(exLogFrom(AA, BB, 0n, 1n)))
	CTLSPEC not(ef(exLogFrom(BB, AA, 0n, 1n)))
	CTLSPEC not(ef(exLogFrom(AA, BB, 5n, 1n)))
	CTLSPEC not(ef(exLogFrom(BB, AA, 5n, 1n)))
	CTLSPEC not(ef(exLogFrom(AA, BB, 10n, 1n)))
	CTLSPEC not(ef(exLogFrom(BB, AA, 10n, 1n)))
	CTLSPEC not(ef(exLogFrom(AA, BB, 0n, 2n)))
	CTLSPEC not(ef(exLogFrom(BB, AA, 0n, 2n)))
	CTLSPEC not(ef(exLogFrom(AA, BB, 5n, 2n)))
	CTLSPEC not(ef(exLogFrom(BB, AA, 5n, 2n)))
	CTLSPEC not(ef(exLogFrom(AA, BB, 10n, 2n)))
	CTLSPEC not(ef(exLogFrom(BB, AA, 10n, 2n)))*/
