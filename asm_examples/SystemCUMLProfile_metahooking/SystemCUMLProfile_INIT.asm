...
/*TimeEvent non � utilizzato per questa simulazione
...
/*Restituisce lo stato pi� interno raggiunto dal thread durante l'attuale stato macchina, 
quindi inizializzando la funzione allo stato iniziale si aggiorner� automaticamente*/
...
� un Powerset, perch� uno stato si pu� comporre di pi� stati*/
controlled downState: Vertex -> Powerset(Vertex)
	
/*Restituisce la sequenza di stati dallo stato pi� interno(dato) a quello pi� esterno(dato)*/
derived upChain: Prod(Vertex, Vertex) -> Seq(Vertex)
	
/*Restituisce la sequenza di stati dallo stato pi� esterno(dato) a quello pi� interno(dato)*/
...
/*Definisce se l'evento in analisi � completo*/
...
/*Ritorna lo stato comune pi� interno nelle gerarchia di contenimento*/
...
/*� il solo diretto sottostato di un Vertice v in una sequenza di vertici*/
...
/*� lo stack che �memorizza�lo stato a cui il controllo deve tornare dopo un�operazione di Break*/
controlled nextAfterBreak: Process -> Seq(Vertex)

/*� lo stack che �memorizza�lo stato a cui il controllo deve tornare dopo un�operazione di Break*/
controlled nextAfterContinue: Process -> Seq(Vertex)

/*Restituisce vero se l�esecuzione dell�ttivit� di uno stato interno restituisce uno stato finale*/
...
/*Viene settata a vero quando uno stato finale � il target di una transizione*/
...
/*Deve ritornare la sequenza di stati composti e la upChain la realizza a partire dallo stato pi� interno (contenuto) t a quello pi� esterno (composto) s*/						
...
then [$s,$t]
       ...
       /*Non verifico in questa sede, la composizione "composta" 
         primo: PERCH� PASSO UN POWERSET DI VERTEX COME ARGOMENTO, mentre la downState restituisce un vertex,
...
//Definisce se una transizione � "elegibile"
...
if isDef(asState($v)) //significa che sicuramente il Vertex � uno State
/*Sono aggiunti gli stati dont_init e static_wait perch� pur essendo stati 
con un valido asState non rientrando in questi casi non consentivano l'esecuzione*/
	...
	else //lo stato v non ha definito asState, quindi � uno pseudostato, quindi sicuramente � uno stato completo
true endif 

function final($v in Vertex) = 
if (isFinal($v) and (isDef(upState($v) )))
/*se il vertex passato � uno stato finale e possiede un upState
  cio� � contenuto in uno stato composto allora ritornalo*/
	  then $v
			else undef 
		endif 
...
//Ritorna la regione(stato) pi� interna che contiene entrambi gli stati
...
function push($v in Vertex, $sv in Seq(Vertex)) = append($sv,$v)//non � implementato il prepend
