...
signature:
	...
	//derived  overallWindowsStatus: Powerset(AirMdA) -> WindowStatus //OPEN, if at least one is open; false otherwise
	derived  overallWindowsStatus: WindowStatus //OPEN, if at least one is open; false otherwise
	//derived overallAQInsideCO2: Powerset(AirMdA) -> Integer
...
definitions:
  ...
  function overallAQInsideCO2 =
	...
	rule r_Air = //Added in refinement for debugging/logging
	...
	rule r_MainAQE =
		if startMainAQE(self) then
			par
				//forall  $a in airManagedByMainAQ(self) do
    ...
    */
	...
	rule r_SaveSensorsData =
		par
			...
			//windowsStatusSaved(self) := overallWindowsStatus(airManagedByMainAQ(self))
			windowsStatusSaved(self) := overallWindowsStatus
			//aQInsideCO2Saved(self):= overallAQInsideCO2(airManagedByMainAQ(self))
	...
	LTLSPEC g(((aQInsideCO2Saved(aq_main)>=3 and not(outsideAqiSaved(aq_main)>=2))) implies f(windows_Setting(aqs_gf) = OPEN))
	LTLSPEC g((outsideAqiSaved(aq_main) > 1 and windowsStatusSaved(aq_main) = OPEN)
				implies f(windows_Setting(aqs_gf) = CLOSED))
	LTLSPEC g(((aQInsideCO2Saved(aq_main)>=3 and not(outsideAqiSaved(aq_main)>=2))) implies f(windows_Setting(aqs_ff) = OPEN))
	LTLSPEC g((outsideAqiSaved(aq_main) > 1 and windowsStatusSaved(aq_main) = OPEN)
				implies f(windows_Setting(aqs_ff) = CLOSED))
