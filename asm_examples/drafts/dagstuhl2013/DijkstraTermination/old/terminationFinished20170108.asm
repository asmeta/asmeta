...
definitions:
   ...
   function stableState = (forall $m in Machine with not(active($m)))
	...
	// Upon transmission of the token
	...
	rule r_rule3_4($m in Machine) =
		...
		and hasToken($m) and isMaster($m) and (machineColor($m) = BLACK or tokenColor= BLACK) then
			par
				r_passToken[$m]
				r_rule4[]
			endpar
	...
	CTLSPEC ag(finished implies stableState)

	main rule r_main =
		//forall $ma in Machine with true do
		choose $ma in Machine with true do
			par
