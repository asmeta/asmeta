		...
		let ($cameraOCself = cameraOC(self)) in
			par
				...
				forall $s in Camera with true do
					par
						slaves($cameraOCself, $s) := false
						newSlave($cameraOCself, $s) := false //??? necessario? Non basta un r_clearSlaves?
					endpar
	...
	macro rule r_selfFailureDetect =
		let ($cameraSCHself = cameraSHC(self)) in
			par
				...
				if stateSHC(self) = FAILED2  then //@P
					if startCam($cameraSCHself) then
						stateSHC(self) := START
					endif
				endif
			endpar
