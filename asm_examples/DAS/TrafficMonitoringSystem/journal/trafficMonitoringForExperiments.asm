asm trafficMonitoringForExperiments
...
signature:
	...
	//Camera system 5
	static c5: Camera
	static trafficMonitor5: TrafficMonitor
	static organizationController5: OrganizationController
	static selfHealingController5: SelfHealingController

	//Camera system 6
	static c6: Camera
	static trafficMonitor6: TrafficMonitor
	static organizationController6: OrganizationController
	static selfHealingController6: SelfHealingController

	//Camera system 7
	static c7: Camera
	static trafficMonitor7: TrafficMonitor
	static organizationController7: OrganizationController
	static selfHealingController7: SelfHealingController

	//Camera system 8
	static c8: Camera
	static trafficMonitor8: TrafficMonitor
	static organizationController8: OrganizationController
	static selfHealingController8: SelfHealingController

	//Camera system 9
	static c9: Camera
	static trafficMonitor9: TrafficMonitor
	static organizationController9: OrganizationController
	static selfHealingController9: SelfHealingController

	//Camera system 10
	static c10: Camera
	static trafficMonitor10: TrafficMonitor
	static organizationController10: OrganizationController
	static selfHealingController10: SelfHealingController
...
definitions:
	...
	function cameraSHC($a in SelfHealingController) =
	   switch($a)
			...
			case selfHealingController5: c5
			case selfHealingController6: c6
			case selfHealingController7: c7
			case selfHealingController8: c8
			case selfHealingController9: c9
			case selfHealingController10: c10
	  endswitch		
	...
	function cameraOC($a in OrganizationController) =
		switch($a)		
			...
			case organizationController5: c5
			case organizationController6: c6
			case organizationController7: c7
			case organizationController8: c8
			case organizationController9: c9
			case organizationController10: c10
		endswitch
				...
				if stateC(c2) != FAILED then
					...
					if stateC(c3) != FAILED then
						...
						if stateC(c4) != FAILED then
							...
							if stateC(c5) != FAILED then
								c5
							else
								if stateC(c6) != FAILED then
									c6
								else
									if stateC(c7) != FAILED then
										c7
									else
										if stateC(c8) != FAILED then
											c8
										else
											if stateC(c9) != FAILED then
												c9
											else
												if stateC(c10) != FAILED then
													c10
												else
													undef
												endif
	...
	LTLSPEC g((stateOC(organizationController4) = MASTER and congested(organizationController4) and m_offer(c4)) implies
				o(stateOC(organizationController4) = MASTER and not(stopCam(c4)) and cong(c4) and not(congested(organizationController4))) )
	LTLSPEC g((stateOC(organizationController5) = MASTER and congested(organizationController5) and not(m_offer(c5)) and s_offer(c5)) implies
				o(stateOC(organizationController4) = MASTER and not(stopCam(c4)) and cong(c4) and not congested(organizationController4)) )
	LTLSPEC g((stateOC(organizationController4) = MASTER and congested(organizationController4) and m_offer(c4)) implies
				o(stateOC(organizationController5) = MASTER and congested(organizationController5) and not(m_offer(c5)) and s_offer(c5)) )

	LTLSPEC g((stateOC(organizationController5) = MASTER and congested(organizationController5) and m_offer(c5)) implies
				o(stateOC(organizationController5) = MASTER and not(stopCam(c5)) and cong(c5) and not(congested(organizationController5))) )
	LTLSPEC g((stateOC(organizationController6) = MASTER and congested(organizationController6) and not(m_offer(c6)) and s_offer(c6)) implies
				o(stateOC(organizationController5) = MASTER and not(stopCam(c5)) and cong(c5) and not congested(organizationController5)) )
	LTLSPEC g((stateOC(organizationController5) = MASTER and congested(organizationController5) and m_offer(c5)) implies
				o(stateOC(organizationController6) = MASTER and congested(organizationController6) and not(m_offer(c6)) and s_offer(c6)) )

	LTLSPEC g((stateOC(organizationController6) = MASTER and congested(organizationController6) and m_offer(c6)) implies
				o(stateOC(organizationController6) = MASTER and not(stopCam(c6)) and cong(c6) and not(congested(organizationController6))) )
	LTLSPEC g((stateOC(organizationController7) = MASTER and congested(organizationController7) and not(m_offer(c7)) and s_offer(c7)) implies
				o(stateOC(organizationController6) = MASTER and not(stopCam(c6)) and cong(c6) and not congested(organizationController6)) )
	LTLSPEC g((stateOC(organizationController6) = MASTER and congested(organizationController6) and m_offer(c6)) implies
				o(stateOC(organizationController7) = MASTER and congested(organizationController7) and not(m_offer(c7)) and s_offer(c7)) )

	LTLSPEC g((stateOC(organizationController7) = MASTER and congested(organizationController7) and m_offer(c7)) implies
				o(stateOC(organizationController7) = MASTER and not(stopCam(c7)) and cong(c7) and not(congested(organizationController7))) )
	LTLSPEC g((stateOC(organizationController8) = MASTER and congested(organizationController8) and not(m_offer(c8)) and s_offer(c8)) implies
				o(stateOC(organizationController7) = MASTER and not(stopCam(c7)) and cong(c7) and not congested(organizationController7)) )
	LTLSPEC g((stateOC(organizationController7) = MASTER and congested(organizationController7) and m_offer(c7)) implies
				o(stateOC(organizationController8) = MASTER and congested(organizationController8) and not(m_offer(c8)) and s_offer(c8)) )

	LTLSPEC g((stateOC(organizationController8) = MASTER and congested(organizationController8) and m_offer(c8)) implies
				o(stateOC(organizationController8) = MASTER and not(stopCam(c8)) and cong(c8) and not(congested(organizationController8))) )
	LTLSPEC g((stateOC(organizationController9) = MASTER and congested(organizationController9) and not(m_offer(c9)) and s_offer(c9)) implies
				o(stateOC(organizationController8) = MASTER and not(stopCam(c8)) and cong(c8) and not congested(organizationController8)) )
	LTLSPEC g((stateOC(organizationController8) = MASTER and congested(organizationController8) and m_offer(c8)) implies
				o(stateOC(organizationController9) = MASTER and congested(organizationController9) and not(m_offer(c9)) and s_offer(c9)) )

	LTLSPEC g((stateOC(organizationController9) = MASTER and congested(organizationController9) and m_offer(c9)) implies
				o(stateOC(organizationController9) = MASTER and not(stopCam(c9)) and cong(c9) and not(congested(organizationController9))) )
	LTLSPEC g((stateOC(organizationController10) = MASTER and congested(organizationController10) and not(m_offer(c10)) and s_offer(c10)) implies
				o(stateOC(organizationController9) = MASTER and not(stopCam(c9)) and cong(c9) and not congested(organizationController9)) )
	LTLSPEC g((stateOC(organizationController9) = MASTER and congested(organizationController9) and m_offer(c9)) implies
				o(stateOC(organizationController10) = MASTER and congested(organizationController10) and not(m_offer(c10)) and s_offer(c10)) )
