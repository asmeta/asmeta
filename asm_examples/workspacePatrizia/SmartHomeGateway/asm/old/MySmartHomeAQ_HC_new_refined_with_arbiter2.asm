asm MySmartHomeAQ_HC_new_refined_with_arbiter2
...
//functions added in refinement
	...
	controlled sgnMainHCPtoE: MainHCMgA -> Boolean 
...
function airLevel ($v in Integer) = 
	...
	function startMainHCE($b in MainHCMgA) =
		sgnMainHCPtoE($b)
		//(sgnMainHCPtoE($b) and sgnHightoMainHC_E($b))
...
rule r_checkAirAdaptationRequired =
	...
	par	
		...
		sgnMainHCPtoE(self):= false
	endpar
	
	rule r_MainHCE =//refined
		if startMainHCE(self) then
			par //Added in refinement
		...
		if startMainHCP(self) then
			par
				forall $a in fromMainHCMtoIntHCM(self) do  //Added in refinement
				   ...
				   endpar
				sgnMainHCPtoE(self):= true //r_MainHCE[]
				r_CleanUp_MainHCP[]
			endpar
		endif
...
default init s0:
    ...
    function sgnMainHCPtoE($a in MainHCMgA) = false
