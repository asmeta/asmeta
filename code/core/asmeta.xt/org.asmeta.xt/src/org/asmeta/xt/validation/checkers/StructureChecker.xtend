...
class StructureChecker {
	...
	def static ErrorType isAsmNameOK(Asm asm) {
		...
		if(!name_no_extension.equals(asm.name)) return new ErrorType(message, feature, code) else return null
	...
	def static ErrorType isMainRuleArityOK(Asm asm) {
		...
		if(Utility.calculateRuleArity(asm.mainrule) != 0) return new ErrorType(message, feature, code) else return null
	}
	...
	def static WarningType isModuleMainRuleOK(Asm asm) {
		...
		if(asm.type.equals('module') && asm.mainrule !== null) new WarningType(message, feature, code) else return null
	}
	...
	def static WarningType isModuleDefaultInitialStateOK(Asm asm) {
		...
		if(asm.type.equals('asm')) return null
		// check if it's a module
		if(asm.type.equals('module') && asm.defaultInitialState !== null) new WarningType(message, feature,
	...
	def static WarningType isModuleInitialStateOK(Asm asm) {
		...
		if(asm.type.equals('asm')) return null
		// check if it's a module
		if(asm.type.equals('module') && asm.initialState.size !== 0) new WarningType(message, feature,
			code) else return null
	...
	def static WarningType haveSomethingToImport(ImportClause importClause) {
		...
		var String message = "The ASM '" + asm.name +
	...
	def static MessageType isExportedListOK(ExportClause exportClause, Signature signature, Body body) {
		...
		if (exportClause.exportAll) {
			...
			// in the asm we have declared some rule or function or domain 
			okExport = ( signature.function !== null && signature.function.size > 0) ||
				( body.ruleDeclaration !== null && body.ruleDeclaration.size > 0) ||
				( signature.domain !== null && signature.domain.size > 0)
			// if there is only the main rule to export ?????
			if (!okExport && body.asm.mainrule !== null){
				message = "Exporting * from a spec with only a main "
				code = ""
				var MessageType m = new WarningType(message, feature, code) 
				return new WarningType(message, feature, code) 
			}
		} else {
			var HashMap<String, String> names = new HashMap
			...
			for (String s : exportClause.exportedList) {
				...
				if(!okExport &&
					( Utility.isFunctionName(s) || Utility.isRule(s) || Utility.isDomainName(s))) okExport = true
			}
		}
		if (!okExport) {
	...
	def static isDefinedOnceOK(DomainDefinition domain_definition) {
		...
		for (DomainDefinition definitions : body.domainDefinition) {
			if (names.containsKey(definitions.definedDomainName) &&
				definitions.definedDomainName.equals(domain_definition.definedDomainName))
				return new ErrorType(message, feature, code)
			else
				names.put(definitions.definedDomainName, definitions.definedDomainName)
		}
	...
	def static ErrorType isBodyOK(FunctionDefinition function_definition) {
		...
		if (!Utility.isCompatible(body_domain, tree)) {
			message = "The domain of the function body (" + body_domain.model.root +
				") must be compatible with the function codomain (" + tree.model.root + ")"
			return new ErrorType(message, feature, code)
		}
	...
	def static ErrorType isDefineOnceOK(DomainInitialization domain_init) {
		...
		for (DomainInitialization init : init_state.domainInitialization) {
			if (names.containsKey(init.initializedDomain) &&
				init.initializedDomain.equals(domain_init.initializedDomain))
				return new ErrorType(message, feature, code)
			else
				names.put(init.initializedDomain, init.initializedDomain)
		}
