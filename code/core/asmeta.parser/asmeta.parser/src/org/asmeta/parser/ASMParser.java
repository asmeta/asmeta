...
public class ASMParser implements ASMParserConstants {
  ...
  public static AsmCollection setUpReadAsm(File f) throws Exception
  {
    ...
    assert AsmetaParserUtility.endsWithAsmetaExtension(f.getName());
    Logger.logInfo("parsing file " + f.getCanonicalPath());
    // reset the domains
    DomainsFactory.eINSTANCE.resetDomains();
    AsmetaParserUtility.reset();
    // BUILD THE MODEL
  ...
  private ASMParser(File f, AsmetaFactory asmPack) throws Exception
  {
    ...
    //}
    // get the filename without ".asm" extension or .asmeta
    filename = f.getName();
...
// if it is escaped by double quotes, remove them
    ...
    try
    {
      ...
      File moduleFile = AsmetaParserUtility.importFile(asmDirLib, moduleName);
...
if (module_expCl != null)
        {
          d = AsmetaParserUtility.search_domName(module_dom, importedID);
          if (d != null)
          {
            impDomains.add(d);
...
if (module_expCl != null)
        {
          f = AsmetaParserUtility.search_funcName(module_func, importedID);
          if (f != null)
          {
            impFunctions.add(f);
            Logger.logDebug(importedID);
...
if (module_expCl != null)
        {
          r = AsmetaParserUtility.search_ruleName(module_rules, importedID);
          if (r != null)
          {
            impRules.add(r);
            Logger.logDebug(importedID);
...
Logger.logDebug("signature:");
    ...
    for (ImportClause impClause : header.getImportClause())
    {
      ...
      {
        inserted = AsmetaParserUtility.insert(fi, declared_Func);
        if (inserted)
        { // DO NOT add the function to the signature
          //insert domain and codomain of the function in the declared Domains
          AsmetaParserUtility.insert(fi.getDomain(), declared_Dom);
          AsmetaParserUtility.insert(fi.getCodomain(), declared_Dom);
...
Logger.logDebug("");
    ...
    while (true) {
      ...
      f = Function();
inserted = AsmetaParserUtility.insert(f, declared_Func);
      if (inserted)
...
//set the reference
      ...
      AsmetaParserUtility.updateVariable(v, inDom);
...
//set the reference
    ...
    else if (func_initVariables.size() == 1)
    {
      ...
      if (func_dom == null) func_dom = AsmetaParserUtility.getBasicDomain(domainsFactory, dom_name);
      if (func_dom == null) func_dom = AsmetaParserUtility.getPredefinedAbstractDomain(domainsFactory, dom_name);
      if (func_dom == null) func_dom = AsmetaParserUtility.getStructuredDomain(domainsFactory, dom_name, header.getSignature());
    ...
    else
    {
      ...
      func_dom = (Domain) AsmetaParserUtility.getStructuredDomain(domainsFactory, dom_name, header.getSignature());
      if (func_dom == null) {if (true) throw new ParseException("Error: The domain" + dom_name + " has not been declared. There not exists a function with this domain.");}
    }
    //check if the function has been declared
    /// attenzione non trova le funzioni importate !!!!
    Function f = AsmetaParserUtility.getFunction(func_name, func_dom, false, new HashMap(), declared_Func);
...
Logger.logDebug("\tagent domain: " + id_dom);
    ...
    if (dom == null) dom = AsmetaParserUtility.getPredefinedAbstractDomain(domainsFactory, id_dom);
...
String rule_name = r_decl.getName();
      //check if the rule has already been declared
      if (AsmetaParserUtility.searchRuleByName(declared_Rules, rule_name, r_decl.getVariable()) == null)
      {
...
//set the reference
    ...
    {
      dom_name = "Prod(".concat(dom_name).concat(")");
      func_dom = AsmetaParserUtility.getStructuredDomain(domainsFactory, dom_name, header.getSignature());
      if (func_dom == null) {if (true) throw new ParseException("Error: The domain" + dom_name + " has not been declared. There not exists a function with this domain.");}
    }
    //check if the function has been declared
    Function f = AsmetaParserUtility.getFunction(func_name, func_dom, false, new HashMap < String, Domain > (), declared_Func);
...
dom = (Domain) declared_Dom.get(idElement);
      if (dom == null) dom = AsmetaParserUtility.getBasicDomain(domainsFactory, idElement);
      if (dom == null) dom = AsmetaParserUtility.getPredefinedAbstractDomain(domainsFactory, idElement);
      if (dom == null) dom = AsmetaParserUtility.getStructuredDomain(domainsFactory, idElement, header.getSignature());
...
if (!funHasArg)
      ...
      {
        func = AsmetaParserUtility.getFunction(idElement, dom, false, new HashMap(), declared_Func);
      ...
      idElement = ID_RULE();
rule = AsmetaParserUtility.getRuleByNameDom(declared_Rules, idElement, null);
      rules.add(rule);
...
/*---------------  AnyDomain  To be done! ------------------*/
  final public AnyDomain AnyDomain() throws ParseException {String name;
    ...
    name = ID_DOMAIN();
AnyDomain d = AsmetaParserUtility.getAnyDomain(domainsFactory, name);
    if (d == null)
    {
      // TODO
...
/*---------------  BasicTD  OK! ------------------*/
  final public BasicTd BasicTD() throws ParseException {String name;
    ...
    jj_consume_token(DOMAIN);
    name = ID_DOMAIN();
BasicTd basicTd = AsmetaParserUtility.getBasicDomain(domainsFactory, name);
...
isDynamic = true;
    ...
    name = ID_DOMAIN();
// try predefined
    AbstractTd abstractTd = AsmetaParserUtility.getPredefinedAbstractDomain(domainsFactory, name);
...
{if ("" != null) return AsmetaParserUtility.getRuleDomain(ruleElemList, defPack, header.getSignature());}
    throw new Error("Missing return statement in function");
}
...
//a StructuredTD can not be defined over a concrete domain  NO LONGER APPLIES!!!
    ...
    jj_consume_token(116);
{if ("" != null) return AsmetaParserUtility.getProduct(prodElemList, defPack, header.getSignature());}
    throw new Error("Missing return statement in function");
}
...
//NO LONGER APPLIES!!!
    ...
    {if ("" != null) return AsmetaParserUtility.getSequence(dom, defPack, header.getSignature());}
    throw new Error("Missing return statement in function");
}
...
//NO LONGER APPLIES!!!
    ...
    {if ("" != null) return AsmetaParserUtility.getPowerset(dom, defPack, header.getSignature());}
    throw new Error("Missing return statement in function");
}
...
//NO LONGER APPLIES!!!
    ...
    {if ("" != null) return AsmetaParserUtility.getBag(dom, defPack, header.getSignature());}
    throw new Error("Missing return statement in function");
}
...
//NO LONGER APPLIES!!!
    ...
    {if ("" != null) return AsmetaParserUtility.getMap(dom1, dom2, defPack, header.getSignature());}
    throw new Error("Missing return statement in function");
}
...
domain = declared_Dom.get(domName);
      if (domain == null) domain = AsmetaParserUtility.getPredefinedDomain(domainsFactory, domName);
      if (domain == null) {
...
arg1 = t;
      String funcName = AsmetaParserUtility.getFunctionName(tok.image);
      TupleTerm tupleT = AsmetaParserUtility.createPair(arg1, arg2, termsPack, defPack, header.getSignature());
      //assert tupleT.getTerms().size() == tupleT.getArity();
      if ((t = AsmetaParserUtility.createFunctionTerm(funcName, tupleT, termsPack, declared_Func, declared_Dom, defPack, header.getSignature())) == null) {if (true) throw new ParseException("Error at ");}
    }
{if ("" != null) return t;}
    throw new Error("Missing return statement in function");
}
...
String funcName = AsmetaParserUtility.getFunctionName(op.image);
      TupleTerm tupleT = AsmetaParserUtility.createPair(arg1, arg2, termsPack, defPack, header.getSignature());
      //assert tupleT.getTerms().size() == tupleT.getArity();
      arg1 = AsmetaParserUtility.createFunctionTerm(funcName, tupleT, termsPack, declared_Func, declared_Dom, defPack, header.getSignature());
...
}
{
  ...
  (
    ...
    )
    {
      ...
      String funcName = AsmetaParserUtility.getFunctionName(tok.image);
      TupleTerm tupleT = AsmetaParserUtility.createPair(arg1, arg2, termsPack, defPack, header.getSignature());
      //assert tupleT.getTerms().size() == tupleT.getArity();      if ((t = AsmetaParserUtility.createFunctionTerm(funcName, tupleT, termsPack, declared_Func, declared_Dom, defPack, header.getSignature())) == null) throw new ParseException("Error at ");
    }
  )?
  {
    return t;
...
arg1 = t;
      ...
      TupleTerm tupleT = AsmetaParserUtility.createPair(arg1, arg2, termsPack, defPack, header.getSignature());
      //assert tupleT.getTerms().size() == tupleT.getArity();
      t = AsmetaParserUtility.createFunctionTerm(funcName, tupleT, termsPack, declared_Func, declared_Dom, defPack, header.getSignature());
      if (t == null)
...
String funcName = AsmetaParserUtility.getFunctionName(op.image);
      TupleTerm tupleT = AsmetaParserUtility.createSingle(arg, termsPack, defPack, header.getSignature());
      //assert tupleT.getTerms().size() == tupleT.getArity();
      if ((t = AsmetaParserUtility.createFunctionTerm(funcName, tupleT, termsPack, declared_Func, declared_Dom, defPack, header.getSignature())) == null) {if (true) throw new ParseException("Error at ");}
    } else if (jj_2_126(2)) {
...
if (agentName != null)
    { //create a new functionTerm representing the agent
      ...
      {
        if ((agent_funT = AsmetaParserUtility.createFunctionTerm("self", null, termsPack, declared_Func, declared_Dom, defPack, header.getSignature())) == null) {if (true) throw new ParseException("Error at ");}
      }
      else
      {
        Term arg = AsmetaParserUtility.createStringT(agentName, termsPack, domainsFactory);
        TupleTerm agentArg = AsmetaParserUtility.createSingle(arg, termsPack, defPack, header.getSignature());
        //assert agentArg.getTerms().size() == agentArg.getArity();
        if ((agent_funT = AsmetaParserUtility.createFunctionTerm("getAgent", agentArg, termsPack, declared_Func, declared_Dom, defPack, header.getSignature())) == null) {if (true) throw new ParseException("Error at ");}
      }
      //insert the functionTerm created as first element of tupleT
      if (tupleT == null)
      { //create a new TupleTerm for the FunctionTerm
        tupleT = AsmetaParserUtility.createSingle(agent_funT, termsPack, defPack, header.getSignature());
        //assert tupleT.getTerms().size() == tupleT.getArity();
      }
      else tupleT.getTerms().add(0, agent_funT);
    }
    if ((t = AsmetaParserUtility.createFunctionTerm(funcName, tupleT, termsPack, declared_Func, declared_Dom, defPack, header.getSignature())) == null)
...
if (agentName != null)
    { //create a new functionTerm representing the agent
      ...
      {
        Term arg = AsmetaParserUtility.createStringT(agentName, termsPack, domainsFactory);
        TupleTerm agentArg = AsmetaParserUtility.createPair(arg, null, termsPack, defPack, header.getSignature());
        if ((agent_funT = AsmetaParserUtility.createFunctionTerm("getAgent", agentArg, termsPack, declared_Func, declared_Dom, defPack, header.getSignature())) == null) {if (true) throw new ParseException("Error at ");}
      }
      //insert the functionTerm created as first element of tupleT
      if (tupleT == null)
      { //create a new TupleTerm for the FunctionTerm
        tupleT = AsmetaParserUtility.createPair(agent_funT, null, termsPack, defPack, header.getSignature());
      }
      else tupleT.getTerms().add(0, agent_funT);
    }
    FunctionTerm f = AsmetaParserUtility.createFunctionTerm(funcName, tupleT, termsPack, declared_Func, declared_Dom, defPack, header.getSignature());
    t = (LocationTerm) f;
...
VariableTerm variable = (VariableTerm) localVar.get(name);
    ...
    { //create the object
      ...
      //set TypeDomain
      AnyDomain anyDomain = AsmetaParserUtility.getAnyDomain(domainsFactory, anyDomainAnyName);
...
symbol = t.image;
    ...
    BasicTd complexDom = AsmetaParserUtility.getBasicDomain(domainsFactory, "Complex");
    //set the reference
...
symbol = symbol.concat(t.image);
    ...
    TypeDomain dom = AsmetaParserUtility.getBasicDomain(domainsFactory, "Real");
    if (dom == null)
    {
...
symbol = symbol.concat(t.image);
    ...
    TypeDomain dom = (TypeDomain) AsmetaParserUtility.getBasicDomain(domainsFactory, "Integer");
    if (dom == null)
    {
...
symbol = t.image;
    ...
    TypeDomain dom = (TypeDomain) AsmetaParserUtility.getBasicDomain(domainsFactory, "Natural");
    if (dom == null)
    {
...
symbol = t.image;
    ...
    TypeDomain dom = (TypeDomain) AsmetaParserUtility.getBasicDomain(domainsFactory, "Char");
    if (dom == null)
    {
...
//PA 2018/07/25
    ...
    StringTerm term = AsmetaParserUtility.createStringT(symbol, termsPack, domainsFactory);
...
symbol = t.image;
    ...
    TypeDomain dom = (TypeDomain) AsmetaParserUtility.getBasicDomain(domainsFactory, "Boolean");
    if (dom == null)
    {
...
Logger.logDebug("match:UndefTerm");
    ...
    TypeDomain dom = (TypeDomain) AsmetaParserUtility.getBasicDomain(domainsFactory, "Undef");
    if (dom == null)
    {
...
//set the tuple arity
    ...
    dom = AsmetaParserUtility.getProduct(prodDomList, defPack, header.getSignature());
    term.setDomain(dom);
...
if (!(elem.getDomain() instanceof IntegerDomain)) natural_step = false;
            step = AsmetaParserUtility.convertToDouble(elem);
...
AsmetaParserUtility.createTermCollection(firstElem, lastElem, elemList, step, natural_step, termsPack, declared_Func, declared_Dom, defPack, header.getSignature());
        } else {
          jj_consume_token(-1);
          throw new ParseException();
        }
...
//set the size
    ...
    {
      elemTD = AsmetaParserUtility.getCommonTD(elemList);
    ...
    SequenceDomain seqDom = AsmetaParserUtility.getSequence(elemTD, defPack, header.getSignature());
...
//constraint 3
      ...
      String key = AsmetaParserUtility.appendInKey(new StringBuffer(), firstElem).toString();
...
//constraint 3
          ...
          key = AsmetaParserUtility.appendInKey(new StringBuffer(), elem).toString();
...
AsmetaParserUtility.createTermCollection(firstElem, lastElem, elemColl, step, natural_step, termsPack, declared_Func, declared_Dom, defPack, header.getSignature());
        } else {
          jj_consume_token(-1);
          throw new ParseException();
        }
...
//set the size
    ...
    {
      elemTD = AsmetaParserUtility.getCommonTD(elemColl);
      if (elemTD == null) {if (true) throw new ParseException("Error: every element of a set must have the same type-domain.\n");}
    }
    PowersetDomain setDom = AsmetaParserUtility.getPowerset(elemTD, defPack, header.getSignature());
...
//create a new pair
        TupleTerm pair = AsmetaParserUtility.createPair(elem1, elem2, termsPack, defPack, header.getSignature());
        //add a new element to the terms list
        pairColl.add(pair);
        //add a new association
        ...
        StringBuffer key = AsmetaParserUtility.appendInKey(new StringBuffer(), elem1);
        key.append("->");
        AsmetaParserUtility.appendInKey(key, elem2);
...
Logger.logDebug("\t->");
        elem2 = Term(localVar, areAllowedNewVar, null);
//create a new pair
          pair = AsmetaParserUtility.createPair(elem1, elem2, termsPack, defPack, header.getSignature());
          //add a new element to the terms list
          pairColl.add(pair);
          //add a new association
          //a_MapTerm_Pair.add(term,pair);
          ...
          key = AsmetaParserUtility.appendInKey(new StringBuffer(), elem1);
          key.append("->");
          AsmetaParserUtility.appendInKey(key, elem2);
...
//set the size
    ...
    MapDomain mapDom = AsmetaParserUtility.getMap(sourceTD, targetTD, defPack, header.getSignature());
    //set setDom as the set TypeDomain
...
//set the bag size
    ...
    BagDomain bagDom = AsmetaParserUtility.getBag(elemTD, defPack, header.getSignature());
...
//check OCL constraints
    ...
    //look for the boolean domain reference in the predefined_Dom HashMap
    TypeDomain td = AsmetaParserUtility.getBasicDomain(domainsFactory, "Boolean");
...
Logger.logDebug("\t(exist\t\tExistTerm beginning");
    ExistsTerm term = termsPack.getFurtherTerms().createExistTerm();
...
Logger.logDebug("\t(exist unique\t\tExistUniqueTerm beginning");
    ExistsUniqueTerm term = termsPack.getFurtherTerms().createExistUniqueTerm();
    List < Term > rangeList = term.getRanges();
    //check if the next variable is already used (look for it in the localVar HashMap)
    String nextVar = getToken(1).image;
...
//set the reference
    ...
    jj_consume_token(120);
// set TypeDomain
    TypeDomain td = AsmetaParserUtility.getPowerset(OCL_Checker.getTypeDomain(t.getDomain()), defPack, header.getSignature());
    term.setDomain(td);
...
Logger.logDebug("\t|");
//create a new pair
    TupleTerm pair = AsmetaParserUtility.createPair(t1, t2, termsPack, defPack, header.getSignature());
    //set the reference
    term.setTerm(pair);
    //set the association
    //X AComprehensionTermTerm a_Comp_Term = termsPack.getFurtherTerms().getAComprehensionTermTerm();
    ...
    jj_consume_token(120);
// set TypeDomain
    TypeDomain td = AsmetaParserUtility.getMap(OCL_Checker.getTypeDomain(t1.getDomain()), OCL_Checker.getTypeDomain(t2.getDomain()), defPack, header.getSignature());
    term.setDomain(td);
...
//set the expression reference
    ...
    jj_consume_token(124);
// set TypeDomain
    TypeDomain td = AsmetaParserUtility.getSequence(OCL_Checker.getTypeDomain(t.getDomain()), defPack, header.getSignature());
    term.setDomain(td);
    //OCL constraint 1 granted for construction
...
//set the reference
    ...
    jj_consume_token(GT);
// set TypeDomain
    TypeDomain td = AsmetaParserUtility.getBag(OCL_Checker.getTypeDomain(t.getDomain()), defPack, header.getSignature());
    term.setDomain(td);
    //OCL constraint 1 granted for construction
...
//create the object
    ...
    term.setDomain(d);
    //set the TypeDomain
    PowersetDomain powersetDom = AsmetaParserUtility.getPowerset(d, defPack, header.getSignature());
    //set references
...
Logger.logDebug("\t>>\t");
    ...
    r = AsmetaParserUtility.getRuleByNameDom(declared_Rules, ruleName, ruleDomList);
    if (r == null)
    {
      {if (true) throw new ParseException("ERROR: Unresolved reference to " + ruleName + AsmetaParserUtility.toString(ruleDomList));}
    }
    //set the rule reference
    term.setRule(r);
    RuleDomain dom = AsmetaParserUtility.getRuleDomain(ruleDomList, defPack, header.getSignature());
...
//look for the rule in the rule HashMap
    RuleDeclaration r = AsmetaParserUtility.getRuleByNameTerm(declared_Rules, ruleName, actualParamList);
    //PA: 15/10/2011
    /*List<Domain> doms = AsmetaParserUtility.buildDomains(actualParamList);
    ...
    if (r == null)
    {
      List < Domain > domains = AsmetaParserUtility.buildDomains(actualParamList);
      Logger.logErr("rules already declared: " + AsmetaParserUtility.toString(declared_Rules));
      {if (true) throw new ParseException("Error: Unresolved reference to " + ruleName + AsmetaParserUtility.toString(domains));}
    }
...
Collection VariablesList = rule.getBoundVar();
    ...
    ReserveDomain Reservedom = (ReserveDomain) AsmetaParserUtility.getPredefinedAbstractDomain(domainsFactory, "Reserve");
    if (Reservedom == null) {if (true) throw new ParseException("Error: The domain Reserve has not been declared.");}
    AsmetaParserUtility.updateVariable(v, dom);
...
//look for the rule in the rule HashMap
    ...
    if (r == null)
    {
      List < Domain > domains = AsmetaParserUtility.buildDomains(actualParamList);
      Logger.logErr("rules already declared: " + AsmetaParserUtility.toString(declared_Rules));
      //Logger.logErr("Asbtract domains:" + AsmetaParserUtility.toString(AsmetaParserUtility.abstractTds.values()));
      {if (true) throw new ParseException("Error: Unresolved reference to " + ruleName + AsmetaParserUtility.toString(domains));}
    }
...
boolean inserted = AsmetaParserUtility.insert(localF, declared_Func);
...
Logger.logDebug("\t");
      localF = LocalFunction();
inserted = AsmetaParserUtility.insert(localF, declared_Func);
      if (inserted) localFunctions.add(localF);
      else {if (true) throw new ParseException("Error: The function " + localF.getName() + ":" + ((localF.getDomain() == null) ? "" : (localF.getDomain().getName() + "->")) + localF.getCodomain().getName() + " has been declared twice.");}
      jj_consume_token(123);
Logger.logDebug("\t[");
...
rule.setBody(body);
    ...
    for (int count = 0; count < localFunctions.size(); count++)
    {
      ...
      AsmetaParserUtility.remove(f, declared_Func);
    }
