...
//			return -1;
	...
	public static boolean R1(UpdateRule r) {
		...
		if (!(compatible(location, updatingTerm))) {
			MSG_ERR = "Error: In an update rule, the term on the right-hand side ("+ new TermPrinter(false).visit(updatingTerm) + ") must be compatible "
					+ "with the one on the left-hand side (" + new TermPrinter(false).visit(location) + ").";
			return false;
		}
		return true;
	}
	...
	public static boolean K5(TermAsRule r) {
		...
		*/
		if (!(r.getTerm().getDomain() instanceof RuleDomain)) {
			MSG_ERR = "Expected a rule for term " + new TermPrinter(true).visit(r.getTerm()) + " domain " + r.getTerm().getDomain();
			return false;
		}
