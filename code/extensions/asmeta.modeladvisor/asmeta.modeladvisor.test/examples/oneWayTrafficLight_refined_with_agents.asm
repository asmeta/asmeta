...
definitions:
	...
	//in ogni stato, per ogni semaforo, deve essere accesa o la luce rossa o la verde
	CTLSPEC ag(goLight(LIGHTUNIT1) xor stopLight(LIGHTUNIT1))
	CTLSPEC ag(goLight(LIGHTUNIT2) xor stopLight(LIGHTUNIT2))
	
	//se un semaforo e' verde l'altro e' rosso, oppure sono tutti e due rossi
	CTLSPEC ag((goLight(LIGHTUNIT2) and stopLight(LIGHTUNIT1)) xor (goLight(LIGHTUNIT1) and stopLight(LIGHTUNIT2)) xor (stopLight(LIGHTUNIT2) and stopLight(LIGHTUNIT1)))

	//proprieta' equivalente al precedente
	CTLSPEC ag(not(goLight(LIGHTUNIT1) and goLight(LIGHTUNIT2)))
	
	//proprieta' di corretta associazione tra le fasi e le luci accese

	//CTLSPEC ag( (phase=STOP1STOP2 or phase=STOP2STOP1) iff    //             (stopLight(LIGHTUNIT1) and stopLight(LIGHTUNIT2)) )
	...
	//proprieta' di corretta transizione tra gli stati
	...
	//se il semaforo e' rosso, prima o poi diventera' verde
