...
//Some special clients (groups) require more than one taxi.
//This specification does not model the movements of the clients and the taxis. 
...
definitions:
	...
	//it exists a three minutes period in which there are not available taxis
	...
	//liveness property: if there are not available taxis, sooner or later at
	//least one taxi will become available
	CTLSPEC ag(numtaxis = 0 implies af(numtaxis > 0))

	//it exists a state in which group GR3 asks all 3 taxis and its request
	//is accepted; in the following state all the 3 taxis are busy
