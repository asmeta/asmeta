...
signature:
	...
	dynamic controlled ultimaGiocataPc1: Giocata //colui che NON implementa la SV
	dynamic controlled ultimaGiocataPc2: Giocata //colui che implementa la SV
...
//	($g > 0 and ($g - ultimaGiocataPc1) <= distanza and $g > ultimaGiocataPc1)
	...
	rule r_sceltaNumero =
		choose $g in Giocata with (($g - ultimaGiocataPc2) <= distanza and $g > ultimaGiocataPc2) do
			par
				ultimaGiocataPc1 := $g
	...
	rule r_controllaStrategiaVincente =
		if(ultimaGiocataPc2 <= distanza and ultimaGiocataPc2 != 0) then
			par
				ultimaGiocataPc2 := obiettivo mod (distanza + 1)
				if(turno = PLAYER1) then
					turno := PLAYER2
				else
					turno := PLAYER1
	...
	rule r_applicaStrategia($p in Players, $q in Players) =
		if(strategiaVincente = $p) then
			par
				ultimaGiocataPc2 := ultimaGiocataPc2 + distanza + 1
				r_controllaVincita[ultimaGiocataPc2+distanza+1,$p,$q]
				firstMance := false
			endpar
		else
			r_sceltaNumero[]
	...
	//questa proprietà dice che il PLAYER1 non giungerà mai in uno stato di vincita
	invariant over statoVincita: ag(not(statoVincita(PLAYER1)=WIN))

	invariant over statoVincita: ef(statoVincita(PLAYER2) = WIN)

	//con questa proprietà verifico che nel momento in cui viene segnalata la vincita al PLAYER1
	//lo stato della vincita dei due player non cambierà più fino al termine della simulazione
	//Questa regola inoltre controlla che al termine di ogni giocata lo stato della vincita assume questa configurazione
	invariant over statoVincita: ag(ef(ag(statoVincita(PLAYER1)=LOSE and statoVincita(PLAYER2)=WIN)))	
...
//	function gamePc = obiettivo mod (distanza + 1)
	...
	function obiettivo = 99	//99 - 60 - 97
	function ultimaGiocataPc1 = 0
	function ultimaGiocataPc2 = 11
