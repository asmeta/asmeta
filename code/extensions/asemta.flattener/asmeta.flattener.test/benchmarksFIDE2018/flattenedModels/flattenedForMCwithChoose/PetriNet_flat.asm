...
definitions:
    ...
    main rule r_Main =
        choose $t in Transition with isEnabled($t) do
            par
                tokens(p1) := plus(tokens(p1),incidenceMatrix(p1,$t))
                tokens(p2) := plus(tokens(p2),incidenceMatrix(p2,$t))
                tokens(p3) := plus(tokens(p3),incidenceMatrix(p3,$t))
                tokens(p4) := plus(tokens(p4),incidenceMatrix(p4,$t))
            endpar
