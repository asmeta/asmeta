...
// FUNCTIONS
	...
	//Rinse dialyzer
	dynamic monitored df_prepared: Boolean //True--> DF is prepared
	...
	dynamic monitored arterial_bolus_confirmed: Boolean //Arterial bolus termination is confirmed
...
// RULE DEFINITIONS
	...
	rule r_insert_param ($inf1 in Integer, $sup1 in Integer, $nextparam1 in Mode, $mon1 in Integer, $contr1 in Integer) = 
		if (($mon1 >= $inf1 ) and ($mon1 <= $sup1 )) then
			par 
				$contr1 := $mon1 
				mode := $nextparam1
			endpar
	...
	rule r_insert_param ($inf2 in Integer, $sup2 in Integer, $nextparam2 in TreatmentParam, $mon2 in Integer, $contr2 in Integer) = 
		if (($mon2 >= $inf2 ) and ($mon2 <= $sup2 )) then
			par 
				$contr2 := $mon2 
				treatmentParam := $nextparam2 
			endpar
		endif
	
	rule r_insert_param ($inf3 in Integer, $sup3 in Integer, $nextparam3 in PatientPhase, $mon3 in Integer, $contr3 in Integer) = 
		if (($mon3 >= $inf3 ) and ($mon3 <= $sup3 )) then
			par 
				$contr3 := $mon3 
				patientPhase := $nextparam3 
			endpar
		endif
		
	rule r_insert_param ($nextparam4 in TreatmentParam, $mon4 in Concentrate, $contr4 in Concentrate) = 
		par 
			$contr4 := $mon4 
			treatmentParam := $nextparam4 
		endpar

	rule r_insert_param ($nextparam5 in TreatmentParam, $mon5 in Boolean, $contr5 in Boolean) = 
		par 
			$contr5 := $mon5 
			treatmentParam := $nextparam5 
		endpar
	
	rule r_insert_param ($nextparam6 in Mode, $mon6 in SyringeType, $contr6 in SyringeType) = 
		par 
			$contr6 := $mon6 
			mode := $nextparam6 
		endpar
				
	rule r_insert_therapy_time ($nextparam7 in TreatmentParam) =
		par
	...
	rule r_insert_heparin_time ($nextparam8 in TreatmentParam) =
		par
	...
	rule r_set_arterial_bolus_volume =
		if (art_bolus_volume >= 0 and art_bolus_volume <= 400) then
			par
