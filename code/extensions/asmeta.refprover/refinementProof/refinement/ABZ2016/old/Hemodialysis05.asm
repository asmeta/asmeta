...
// FUNCTION DEFINITIONS
	...
	function errorePerBYPASS = error(TEMP_HIGHT) or error(TEMP_LOW) or error(DF_PREP) or error(UF_DIR) or alarm(UF_VOLUME_ERR)
	...
	function error_bp =
		(error(BP_NO_FLOW) = true or error(BP_ROTATION_DIR) = true)
	
	function error_rein_press = 
		(error(REIN_VP_UP) = true or error(REIN_AP_LOW) = true or error(SAD_ERR) = true)
		
	function error_therapy =
		(error(INIT_VP_UP) = true or error(INIT_VP_LOW) = true or error(INIT_AP_UP) = true or error(INIT_AP_LOW) = true or error(SAD_ERR) = true)
...
// RULE DEFINITIONS
	...
	rule r_check_temp_hight = 
		if (error(TEMP_HIGHT) = false) then
			if (current_temp > 41) then	
				par
					error(TEMP_HIGHT) := true
					alarm(TEMP_HIGHT) := true
	...
	rule r_check_bp_no_flow_err = 
		if (error(BP_NO_FLOW) = false) then
			if ((detected_blood_flow = false) and (passedSec(120) = true)) then	
				par
	...
	rule r_check_conn_rein_press_up_limit ($err in ErrorAlarmType, $currentP in Integer, $uplimit in Integer, $sec in Integer)= 
		if (error($err) = false) then
			if ($currentP > $uplimit and passedSec($sec) = true) then
				par
					error($err) := true
					alarm($err) := true
					//bp_status := STOP
	...
	rule r_check_conn_rein_press_low_limit ($err in ErrorAlarmType, $currentP in Integer, $lowlimit in Integer, $sec in Integer) = 
		if (error($err) = false) then
			if ($currentP < $lowlimit and passedSec($sec) = true) then
				par
					error($err) := true
					alarm($err) := true
					//bp_status := STOP
	...
	rule r_error_temp_hight =
		if (error (TEMP_HIGHT) = true and alarm (TEMP_HIGHT) = false) then
			if (current_temp <= 41) then	
				//par
					error (TEMP_HIGHT) := false
	...
	rule r_error_BP_less_flow =
		if (error (BP_LESS_FLOW) = true and alarm (BP_LESS_FLOW) = false) then
			par
				if (blood_flow_error_bf_less = true) then
				...
				if (th_time_error_bf_less = true) then
	...
	rule r_error_df_prep =
		if (error (DF_PREP) = true and alarm (DF_PREP) = false) then
			if (error_bicarbonate_resolve = true) then
				//par
					error(DF_PREP) := false
					//machine_state := MAIN_FLOW
		...
		if (reset_alarm = true) then
			par
				forall $alarmon in ErrorAlarmType with alarm($alarmon)= true do
					alarm($alarmon) := false
				if (alarm(UF_VOLUME_ERR) = true) then
