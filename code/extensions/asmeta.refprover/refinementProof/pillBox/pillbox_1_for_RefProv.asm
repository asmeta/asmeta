...
definitions:
	...
	function timeDiffOver600 = (systemTime-drawerTimer>1)
	...
	CTLSPEC ag(((redLed = ON and not opened and openSwitch) or (redLed = ON and systemTime-drawerTimer=1 and not(opened))) implies ax(redLed = BLINKING))
	// The red light will change value after 10 minutes if the patient doesn't take the pill
	CTLSPEC ag((redLed = ON and systemTime-drawerTimer=1 and not(opened)) implies ax(redLed = BLINKING))
	CTLSPEC ag((redLed = BLINKING and systemTime-drawerTimer>1 and not(openSwitch)) implies ax(redLed = OFF))
	// If the patient takes the pill, red light will turn-off
	CTLSPEC ag((time_consumption<systemTime and not requestSatisfied and opened and not(openSwitch) and not(redLed = OFF) and not(systemTime-drawerTimer>=1)) implies ax(redLed = OFF))
