asm ClientDisplayOutputWithCookie
	...
	/*function modernizr($d in Device, $k in Key) =
		...
		function modernizrSub($d in Device, $k in Key, $c in SubKey) =
											switch $d
												...
												endswitch*/
		//function modernizr($d in Device, $k in Key) =
		//									if ($d = device2 and ($k = CANVAS or $k = AUDIO)) then TRUE endif
		function modernizr($d in Device, $k in Key) = at({(device2, CANVAS) -> YES, (device2, AUDIO) -> YES, (device2, TEXTSHADOW) -> NO}, ($d,$k)) 	
		//function modernizrSub($d in Device, $k in Key, $c in SubKey) =
											//if ($d = device2 and $k = AUDIO and ($c = MP3)) then TRUE endif
		function modernizrSub($d in Device, $k in Key, $c in SubKey) =
					at({(device2, AUDIO, OGG) -> NO, (device2, AUDIO, MP3) -> YES}, ($d,$k,$c))
