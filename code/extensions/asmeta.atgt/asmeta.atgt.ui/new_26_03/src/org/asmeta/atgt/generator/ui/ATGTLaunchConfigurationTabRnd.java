...
public class ATGTLaunchConfigurationTabRnd extends ATGTLaunchConfigurationTab {
	...
	public void initializeFrom(ILaunchConfiguration configuration) {
		...
		try {
			// Load persisted values (or defaults if missing)
			int nSteps = configuration.getAttribute(CONFIG_NSTEPS, N_STEPS_DEFAULT);
			int nTestsVal = configuration.getAttribute(CONFIG_NTESTS, N_TESTS_DEFAULT);

			// Clamp to spinner ranges (defensive)
			nSteps = Math.max(spinnerNsteps.getMinimum(), Math.min(spinnerNsteps.getMaximum(), nSteps));
			nTestsVal = Math.max(nTests.getMinimum(), Math.min(nTests.getMaximum(), nTestsVal));
