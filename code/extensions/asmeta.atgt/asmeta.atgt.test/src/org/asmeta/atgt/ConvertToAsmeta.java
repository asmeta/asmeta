...
public class ConvertToAsmeta {
	...
	public void convertPHDToASMETAFlat() throws Exception {
		...
		String s = "";
		int i=0;
		for (; (s=fin.readLine())!=null; i++) if (i>0) {
			if (s.startsWith("Set of Sequences :")) break;
			s = s.toUpperCase();
			String[] st = s.split(",\t");
		...
		fin.close();
		for (int j=1; j<i; j++) System.out.print("r_"+j+"[] ");
	}
	...
	public void convertPHDToASMETAStruct() throws Exception {
		...
		String rule = "";
		for (int i=0; (s=fin.readLine())!=null; i++) if (i>0) {
			s = s.toUpperCase();
			String[] st = s.split(",\t");
