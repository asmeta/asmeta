...
public class environment {
    ...
    public static void main(String[] args) {
        ...
        Properties props;
        try (InputStream in = environment.class.getClassLoader()
                .getResourceAsStream("configs/digitalTwinExample/zmq_config_DigitalTwinPIPE.properties")) {

            if (in == null) {
                throw new RuntimeException("zmq_config_digitaltwin_unified.properties not found in classpath under configs/digitalTwinExample/");
            }
         ...
         // Extract ONLY environment.* (including common.* if they exist)
            ...
            throw new RuntimeException("Cannot load zmq_config_digitaltwin_unified.properties", e);
        }
