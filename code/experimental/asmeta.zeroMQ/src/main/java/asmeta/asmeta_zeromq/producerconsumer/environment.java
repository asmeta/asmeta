...
public class environment {
	...
	public static void main(String[] args) {
		...
		try (InputStream in = environment.class.getClassLoader()
				.getResourceAsStream("configs/producerconsumer/zmq_config_environment.properties")) {
			if (in == null)
				throw new RuntimeException(
						"zmq_config_environment.properties not found in classpath under producerconsumer/");
			props.load(in);
		...
		try (ZContext context = new ZContext()) {
			...
			try {
				// attesa di 1 secondo per dare tempo ai subscriber di connettersi
				Thread.sleep(1000);
