...
public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
				public void run() {
						try {
							Counter_Win window = new Counter_Win();
...
private void initialize() {
		...
		frame = new JFrame();
		//Titolo 
		frame.setTitle("Counter ASM -> Java");
		...
		JLabel VariabileControlled_Text_1 = new JLabel("counter");
		...
		JLabel VariabileMonitored_text_1 = new JLabel("signal");
		...
		try {
			VariabileMonitored_Valore_1.setText(esecuzione.signal.Value.toString());
		}
		...
		btnEsegui.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
						...
						try {
							VariabileControlled_Valore_1.setText(esecuzione.counter.get().toString());
						...
						esecuzione.signal.Value = true;
						else
						esecuzione.signal.Value = false;
