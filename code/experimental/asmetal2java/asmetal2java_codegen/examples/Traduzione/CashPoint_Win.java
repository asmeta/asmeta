...
public class CashPoint_Win {

    CashPoint esecuzione;
    int stato = 0;
    JFrame frame;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					CashPoint_Win window = new CashPoint_Win();
	 ...
	 */
	public CashPoint_Win() {
		
		esecuzione = new CashPoint();
		initialize();
	}
	...
	private void initialize() {
		    ...
		    frame = new JFrame();
			//Titolo 
			frame.setTitle("CashPoint ASM -> Java");
			...
			//funzione per la dichiarazione iniziale delle variabili
					...
					JLabel VariabileControlled_Text_1 = new JLabel("Account");
					...
					JLabel VariabileControlled_Text_2 = new JLabel("Card");
			...
			System.out.println("\nElenco valori per currentDate :");
			for(int i=0; i< esecuzione.Date_lista.size(); i++)
			System.out.println( (i+1) + " -> " +esecuzione.Date_lista.get(i));
			
			
			JLabel VariabileMonitored_text_1 = new JLabel("currentDate");
			...
			try {
					VariabileMonitored_Valore_1.setText(esecuzione.currentDate.Value.toString());
					}
			...
			public void actionPerformed(ActionEvent e) {
				...
				//"traduzione" della funzione printControlled(esecuzione) del file _Exe
						...
						try{
						VariabileControlled_Valore_1.setText(Account_val);
						...
						try{
						VariabileControlled_Valore_2.setText(Card_val);
				...
				//"traduzione" della funzione askMonitored(esecuzione) del file _Exe
				   ...
				   if(sup1<= 0 || sup1> esecuzione.Date_Class.size())
				   esecuzione.currentDate.set(esecuzione.Date_Class.get(0));
				   else
				   esecuzione.currentDate.set(esecuzione.Date_Class.get(sup1-1));
