...
class SIS_Exe {
	...
	static void printControlled(SIS esecuzione) {
		...
		System.out.println("waterpressure = " + esecuzione.waterpressure.get().value);
		System.out.println("pressure = " + esecuzione.pressure.oldValue.name());
		System.out.println("overridden = " + esecuzione.overridden.get());
		
		System.out.println("safetyInjection = " + esecuzione.safetyInjection.oldValue.name());
		
		}
		
	static void askMonitored(SIS esecuzione) {
		
		System.out.print("Inserire un numero per indicare l'enumerativo per reset "+ 
		...
		for(;;) {
			         int x;
			            ...
			            if (resetControllo.isEmpty()) break;
			            try{
		...
		System.out.print("Inserire un numero per indicare l'enumerativo per block "+ 
		esecuzione.Switch_lista.toString() +":  ");
		...
		for(;;) {
			         int x;
			            ...
			            if (blockControllo.isEmpty()) break;
			            try{
		...
		for(;;) {
			         int x;
			            ...
			            if (deltaControllo.isEmpty()) break;
			            try{
