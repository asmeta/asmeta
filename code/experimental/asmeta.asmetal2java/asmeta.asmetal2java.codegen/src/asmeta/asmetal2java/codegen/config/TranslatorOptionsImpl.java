...
/**
 * contains the translation options used by the generators to decide the
...
public class TranslatorOptionsImpl implements TranslatorOptions {
	...
	/**
	 * Indicates to generate an executable of the generated Java class with a
	 * Graphical User Interface (GUI).
	 */
	private boolean generateWin;

	/**
	 * Indicates to generate a class designed for generating tests with Evosuite for
	...
	/**
	 * Indicates to copy the asm specification files to another folder to be
	 * processed.
	 */
	...
	/**
	 * A map that associates property names with actions that modify the
	 * corresponding boolean fields.
	 *
	 * This map is used to replace a switch-case block, simplifying the logic for
	 * setting property values by dynamically invoking the corresponding setter
	 * action.
	 */
	...
	public TranslatorOptionsImpl() {
		...
		this.export = true;
		this.copyAsm = false;
		this.ignoreDomainException = false;
		mapperSetup();
	}

	/**
	 * Constructs a {@code TranslatorOptions} instance with the specified option
	 * values (Considering only the specific properties related to the translation
	 * process).
	 *
	...
	/**
	 * Initializes the mapping between property names and their corresponding
	 * actions.
	 */
	private void mapperSetup() {
		...
		this.propertyMapper.put(IGNORE_NOT_SUPPORTED_DOMAIN_EXCEPTION, value -> this.ignoreDomainException = value);
	}
	...
	public String getDescription() {
		return "use value for given translator property (the default value is in brackets):\n" + " -D"
				...
				+ ModeConstantsConfig.GENERATE_EXE + " = true/(false) : generate a java class for execution.\n" + " -D"
				+ ModeConstantsConfig.GENERATE_WIN + " = true/(false) : generate an executable java class with a GUI.\n"
				+ " -D" + ModeConstantsConfig.TEST_GEN
				+ " = true/(false) : generate a specific java class designed for test generation with Evosuite.\n"
				+ " -D" + COVER_OUTPUTS_OPTION + " = true/(false) : cover the outputs in the testGen class.\n" + " -D"
				+ COVER_RULES_OPTION + " = (true)/false : cover the rules in the testGen class.\n" + " -D"
				+ EXPORT_OPTION + " = (true)/false : export the generated file into the output folder.\n" + " -D"
				+ COPY_ASM_OPTION + " = true/(false) : copy the amseta spec file to another folder for processing.\n"
				+ " -D" + IGNORE_NOT_SUPPORTED_DOMAIN_EXCEPTION
				+ " = true/(false) : do not stop the execution if a domain is not supported by the ATG class \n"
				+ " Note: Please use " + ModeConstantsConfig.TRANSLATOR + ", " + ModeConstantsConfig.COMPILER + ", "
				+ ModeConstantsConfig.GENERATE_EXE + ", " + ModeConstantsConfig.GENERATE_WIN + " and "
				+ ModeConstantsConfig.TEST_GEN + " options only if you have selected the -mode custom option.";
	}
