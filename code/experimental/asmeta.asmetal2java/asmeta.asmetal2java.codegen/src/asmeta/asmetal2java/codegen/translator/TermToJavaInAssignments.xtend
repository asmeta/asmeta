			...
			for (var i = 0; i < object.comparingTerm.size; i++) {
				if (i == 0){
					sb.append(
						...
						«""»	if(«new DomainToJavaString(res).visit(object.comparedTerm.domain)».toString(«visit(object.comparedTerm)»).equals("«visit(object.comparingTerm.get(i))»"))
								a  «visit(object.resultTerms.get(i))»;
					''')
				} else {
					sb.append(
								'''
						«""»	else if(«new DomainToJavaString(res).visit(object.comparedTerm.domain)».toString(«visit(object.comparedTerm)»).equals("«visit(object.comparingTerm.get(i))»"))
								a  «visit(object.resultTerms.get(i))»;
					''')
				}
			}
