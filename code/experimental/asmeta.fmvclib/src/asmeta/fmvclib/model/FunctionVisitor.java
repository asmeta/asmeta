...
public class FunctionVisitor {
	 ...
	 */
	protected String visit(String function, List<Term> argsTerm) {
	...
	private String visitRangeFunction(List<Term> argsTerm) {
		String str1 = env.tv.visit(argsTerm.get(0));
		String str2 = env.tv.visit(argsTerm.get(1));
		if(Util.isNumber(str1) && Util.isNumber(str2)) {
			if(Integer.parseInt(str1) <= Integer.parseInt(str2)) {
				...
				throw new RuntimeException("Empty range in range function.");
			}
			
		}
		else {
			throw new RuntimeException("The range function must be used only with numbers.");
		}
	}

	private String visitCtlOperatorN(List<Term> argsTerm, String ctlOperator) {
		...
		if(Util.isNumber(nThState)) {
			...
			if(n <=0) {
				throw new RuntimeException("You must specify an integer number greater than 1.");
			}
			else if (n == 1) {
				throw new RuntimeException("You should use ex(property), insted of exN(property, 1).");
			}
			else {
