...
public class CompositionGUI extends JFrame {
	 ...
	 */
	public static void main(SimulationContainer contInstance, CompositionTreeNode compositionTree) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					CompositionGUI window = new CompositionGUI(contInstance, compositionTree);
	...
	public CompositionGUI(SimulationContainer contInstance, CompositionTreeNode compositionTree) {
		...
		for(int i = 1; i < compositionTree.getAllChildrenModels().size(); i++) {
			AsmetaModel compositionModel = compositionManager.getModelFromModelList(compositionTree.getAllChildrenModels().get(i).getModelName(), contInstance.getSimulatorId()); 
	...
	public static void addTab(int tabModelID, int tabSimContainerID) {
		if(tabbedPane != null) {
			modelCounter++;
			CompositionPanel newTab = new CompositionPanel(compositionManager.getModelFromModelList(tabModelID, tabSimContainerID));
