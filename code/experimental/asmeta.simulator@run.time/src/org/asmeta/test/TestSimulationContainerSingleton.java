 ...
 * The Class Test1.
 */
public class TestSimulationContainerSingleton {
...
//===================================START OF STARTEXE=============================================
	...
	// ABBIAMO MODIFICATO IL TIPO DI RETURN DI CHECKSAFETY PER RISOLVERE UN PROBLEMA
	// DI PERDITA DEL VALORE DI ROUT NEL CASO
	// LO STATO UNSAFE VENGA GIA' TROVATO ALL'INTERNO DI CHECKSAFETY, NON SAPPIAMO
	// SE PERO' IL TIPO PRECEDENTE ERA UTILIZZATO
...
//===================================START RUN WITH MONITORING=============================================
	...
	public void runT3() throws Exception {
		...
		Map<String, String> monitored = new HashMap<String, String>();
		monitored.put("operation", "ALLUMER");
		assertTrue(new RunOutput(Esit.UNSAFE, "the id is not found")
				.equalsMessage(imp.runStepTimeout(-1, monitored, 1000)));
	}
...
//===================================START RUN UNTIL EMPTY TIMEOUT WITH MONITORING=============================================		
	...
	public void runUT3() throws Exception {
		...
		monitored.put("operation", "ALLUMER");
		assertTrue(new RunOutput(Esit.UNSAFE, "the id is not found")
				.equalsMessage(imp.runUntilEmptyTimeout(-1, monitored, 1000)));
	}
...
//===================================START RUN UNTIL EMPTY WITH MONITORING=============================================		
	...
	public void runU3() throws Exception {
		...
		monitored.put("operation", "ALLUMER");
		assertTrue(
				new RunOutput(Esit.UNSAFE, "the id is not found").equalsMessage(imp.runUntilEmpty(-1, monitored, max)));
	}
...
//===================================START RUN UNTIL EMPTY TIMEOUT WITHOUT MONITORING===================================
	...
	public void runUWT2() throws Exception {
		...
		imp.init(3);
		assertTrue(new RunOutput(Esit.UNSAFE, "the id is not found")
				.equalsMessage(imp.runUntilEmptyTimeout(-1, max, 1000)));
	}
...
//=========================Inizio Transaction Test=============================================================
	/*
	 * @Test //FIX ALL ASSERT .EQUALS INTO EQUALSMESSAGE if used public void
	 * Transaction1() { System.out.
	 * println(" |||||||||||||||||||||  TEST TRANSACTION1 (RUNSTEP) |||||||||||||||||||||||||||||||||||||||||||||"
	 * ); Queue<Map<String, String>> tail= new LinkedList<>(); Map<String, String>
	 * locationValue1 = new HashMap<String, String>(); Map<String, String>
	 * locationValue2 = new HashMap<String, String>(); Map<String, String>
	 * locationValue3 = new HashMap<String, String>(); Map<String, String>
	 * locationValue4 = new HashMap<String, String>(); Map<String, String>
	 * locationValue5 = new HashMap<String, String>(); Map<String, String>
	 * locationValue6 = new HashMap<String, String>(); Map<String, String>
	 * locationValue7 = new HashMap<String, String>(); Map<String, String>
	 * locationValue8 = new HashMap<String, String>(); Map<String, String>
	 * locationValue9 = new HashMap<String, String>();
	 * 
	 * locationValue1.put("operation", "ALLUMER"); tail.add(locationValue1);
	 * 
	 * locationValue2.put("operation", "ETEINDRE"); tail.add(locationValue2);
	 * 
	 * locationValue3.put("operation", "LANCER"); tail.add(locationValue3);
	 * 
	 * locationValue4.put("operation", "PAUSE"); tail.add(locationValue4);
	 * 
	 * locationValue5.put("operation", "OUVRIR"); tail.add(locationValue5);
	 * 
	 * locationValue6.put("operation", "FERMER"); tail.add(locationValue6);
	 * 
	 * locationValue7.put("operaion", "MOINS"); tail.add(locationValue7);
	 * 
	 * locationValue8.put("operation", "PLUS"); tail.add(locationValue8);
	 * 
	 * locationValue9.put("operation", "TERMINER"); tail.add(locationValue9);
	 * 
	 * 
	 * String model ="examples/Lavatrice.asm"; SimulationContainer imp =
	 * SimulationContainer.getInstance(); imp.init(3); int id =
	 * imp.startExecution(model); //assertTrue( new RunOutput(Esit.UNSAFE,
	 * "everything okays").equals(imp.runStepTransaction(id,tail,model))); }
	 * //ferryman rununtilempty
	 * 
	 * @Test public void Transaction2() { System.out.
	 * println(" |||||||||||||||||||||  TEST TRANSACTION2 MANUAL |||||||||||||||||||||||||||||||||||||||||||||"
	 * ); String model = "examples/ferrymanSimulator_raff1.asm"; SimulationContainer
	 * imp = SimulationContainer.getInstance(); imp.init(1); Map<String, String>
	 * monitored = new HashMap<String, String>(); int id =
	 * imp.startExecution(model); monitored.put("carry", "GOAT");
	 * imp.runUntilEmpty(id, monitored); monitored.clear();monitored.put("carry",
	 * "FERRYMAN"); imp.runUntilEmpty(id, monitored);
	 * monitored.clear();monitored.put("carry", "CABBAGE"); imp.runUntilEmpty(id,
	 * monitored); monitored.clear();monitored.put("carry", "GOAT");
	 * imp.runUntilEmpty(id, monitored); monitored.clear();monitored.put("carry",
	 * "WOLF"); imp.runUntilEmpty(id, monitored);
	 * monitored.clear();monitored.put("carry", "FERRYMAN"); imp.runUntilEmpty(id,
	 * monitored); monitored.clear();monitored.put("carry", "GOAT"); assertTrue( new
	 * RunOutput(Esit.SAFE, "").equals(imp.runUntilEmpty(id, monitored)));
	 * 
	 * } //ferryman ciclo funzionante
	 * 
	 * @Test public void Transaction3() { System.out.
	 * println(" |||||||||||||||||||||  TEST TRANSACTION3 MANUAL |||||||||||||||||||||||||||||||||||||||||||||"
	 * ); String model = "examples/ferrymanSimulator_raff1.asm"; SimulationContainer
	 * imp = SimulationContainer.getInstance(); imp.init(1); Map<String, String>
	 * monitored = new HashMap<String, String>(); int id =
	 * imp.startExecution(model); monitored.put("carry", "GOAT"); imp.runStep(id,
	 * monitored); monitored.clear();monitored.put("carry", "FERRYMAN");
	 * imp.runStep(id, monitored); monitored.clear();monitored.put("carry",
	 * "CABBAGE"); imp.runStep(id, monitored);
	 * monitored.clear();monitored.put("carry", "GOAT"); imp.runStep(id, monitored);
	 * monitored.clear();monitored.put("carry", "WOLF"); imp.runStep(id, monitored);
	 * monitored.clear();monitored.put("carry", "FERRYMAN"); imp.runStep(id,
	 * monitored); monitored.clear();monitored.put("carry", "GOAT"); assertTrue( new
	 * RunOutput(Esit.SAFE, "").equals(imp.runStep(id, monitored))); } //ferryman
	 * transazione runstep
	 * 
	 * @Test public void Transaction4() { System.out.
	 * println(" |||||||||||||||||||||  TEST TRANSACTION4 RUNSTEP |||||||||||||||||||||||||||||||||||||||||||||"
	 * ); String model = "examples/ferrymanSimulator_raff1.asm"; Map<String, String>
	 * lv1 = new HashMap<String, String>(); Map<String, String> lv2 = new
	 * HashMap<String, String>(); Map<String, String> lv3 = new HashMap<String,
	 * String>(); Map<String, String> lv4 = new HashMap<String, String>();
	 * Map<String, String> lv5 = new HashMap<String, String>(); Map<String, String>
	 * lv6 = new HashMap<String, String>(); Map<String, String> lv7 = new
	 * HashMap<String, String>(); lv1.put("carry", "GOAT"); lv2.put("carry",
	 * "FERRYMAN"); lv3.put("carry", "CABBAGE"); lv4.put("carry", "FERRYMAN");
	 * lv5.put("carry", "WOLF"); lv6.put("carry", "FERRYMAN"); lv7.put("carry",
	 * "GOAT"); Queue<Map<String, String>> tail= new LinkedList<>();
	 * tail.add(lv1);tail.add(lv2);tail.add(lv3);tail.add(lv4);tail.add(lv5);tail.
	 * add(lv6);tail.add(lv7); SimulationContainer imp =
	 * SimulationContainer.getInstance(); imp.init(1); Map<String, String> monitored
	 * = new HashMap<String, String>(); int id = imp.startExecution(model);
	 * //assertTrue( new RunOutput(Esit.UNSAFE,
	 * "Invalid Invariant").equalsMessage(imp.runTransaction(id, tail))); } //ciclo
	 * lavatrice funzionante
	 * 
	 * @Test public void Transaction5() { System.out.
	 * println(" |||||||||||||||||||||  TEST TRANSACTION5  |||||||||||||||||||||||||||||||||||||||||||||"
	 * ); String model = "examples/Lavatrice.asm"; SimulationContainer imp =
	 * SimulationContainer.getInstance(); imp.init(1); Map<String, String> monitored
	 * = new HashMap<String, String>(); int id = imp.startExecution(model);
	 * monitored.put("operation", "OUVRIR"); imp.runStep(id, monitored);
	 * monitored.clear();monitored.put("operation", "FERMER"); imp.runStep(id,
	 * monitored); monitored.clear();monitored.put("operation", "ALLUMER");
	 * imp.runStep(id, monitored); monitored.clear();monitored.put("operation",
	 * "PLUS"); imp.runStep(id, monitored);
	 * monitored.clear();monitored.put("operation", "LANCER"); imp.runStep(id,
	 * monitored); monitored.clear();monitored.put("operation", "TERMINER");
	 * assertTrue( new RunOutput(Esit.SAFE, "").equals(imp.runStep(id, monitored)));
	 * } //lavatrice transazione rununtilempty
	 * 
	 * @Test public void Transaction6() { System.out.
	 * println(" |||||||||||||||||||||  TEST TRANSACTION6  |||||||||||||||||||||||||||||||||||||||||||||"
	 * ); String model = "examples/Lavatrice.asm"; Map<String, String> lv1 = new
	 * HashMap<String, String>(); Map<String, String> lv2 = new HashMap<String,
	 * String>(); Map<String, String> lv3 = new HashMap<String, String>();
	 * Map<String, String> lv4 = new HashMap<String, String>(); Map<String, String>
	 * lv5 = new HashMap<String, String>(); Map<String, String> lv6 = new
	 * HashMap<String, String>(); lv1.put("operation", "OUVRIR");
	 * lv2.put("operation", "FERMER"); lv3.put("operation", "ALLUMER");
	 * lv4.put("operation", "PLUS"); lv5.put("operation", "LANCER");
	 * lv6.put("operation", "TERMINER"); Queue<Map<String, String>> tail= new
	 * LinkedList<>();
	 * tail.add(lv1);tail.add(lv2);tail.add(lv3);tail.add(lv4);tail.add(lv5);tail.
	 * add(lv6); SimulationContainer imp = SimulationContainer.getInstance();
	 * imp.init(1); int id = imp.startExecution(model); //assertTrue( new
	 * RunOutput(Esit.SAFE, "").equals(imp.runTransaction(id, tail))); }
	 */
...
//=========================Inizio Asm Test===============================================================
	...
	public void testAddInvariant() throws Exception {
		...
		result = imp.addInvariant(id,
				"invariant over position: position(GOAT)=position(CABBAGE) implies position(GOAT)=position(FERRYMAN)");
		// result=imp.addInvariant(id,"invariant che da parser error");
		System.out.println(imp.viewListInvariant(id));
		// result = imp.addInvariant(id,"invariant over position:
		// position(GOAT)=position(CABBAGE) implies position(GOAT)=position(FERRYMAN)");
		// imp.removeInvariant(id,"invariant over
		// position:position(GOAT)=position(CABBAGE) implies
		// position(GOAT)=position(FERRYMAN)");
		/*
		 * if(result > 0) System.out.println("Everything goes well"); else
	...
	public void testUpdateInvariant() throws Exception {
		...
		imp.init(1);
		int id = imp.startExecution(model);
		result = imp.updateInvariant(id,
				"invariant over position: position(GOAT)=position(CABBAGE) implies position(WOLF)=position(FERRYMAN)",
				"invariant over position: position(GOAT)=position(CABBAGE) implies position(GOAT)=position(FERRYMAN)");
		if (result == 1)
	...
	public void testRemoveInvariant() throws Exception {
		...
		imp.init(1);
		int id = imp.startExecution(model);
		boolean result = imp.removeInvariant(id,
				"invariant over position: position(GOAT)=position(CABBAGE) implies position(GOAT)=position(FERRYMAN)");
		// System.out.println(imp.viewListInvariant(id));
		rigenera();
		assertTrue(result);
	...
	public void testAddInvariantSimulation() throws Exception {
		...
		result = imp.addInvariant(3,
				"invariant over position: position(GOAT)=position(CABBAGE) implies position(GOAT)=position(FERRYMAN)");
		System.out.println(imp.getLoadedIDs());
		rigenera();
		assertTrue(result == -7);
		/*
		 * monitored.put("carry", "WOLF"); imp.runStep(id, monitored, model);
		 */
		/*
		 * monitored.put("carry", "CABBAGE"); imp.runStep(id, monitored, model);
		 * monitored.put("carry", "FERRYMAN"); imp.runStep(id, monitored, model);
		 */

	}

	// Made to check if the simulation requires all monitored variable or can run
	// without some in particular cases
	@Test
	public void missingMonitoredTest() {
		...
		imp.init(1);
		int id = imp.startExecution(model);
		monitored.put("lightMon", "FLASHING");
		monitored.put("gateMon", "OPENED");
		monitored.put("event", "LIGHT");
		imp.runStep(id, monitored);
		monitored.clear();
		monitored.put("gateMon", "CLOSING");
		monitored.put("event", "GATE");
		imp.runStep(id, monitored);
		monitored.clear();
		monitored.put("gateMon", "CLOSED");
		monitored.put("event", "GATE");
		imp.runStep(id, monitored);
		monitored.clear();
		monitored.put("gateMon", "OPENING");
		monitored.put("event", "GATE");
		imp.runStep(id, monitored);
		monitored.clear();
		monitored.put("gateMon", "OPENED");
	...
	// Test===============================================================
	/*
	 * @Test public void testGUI() { InvariantGUI.main(null); }
	 */
