...
/**
 * @author Federico Rebucini Class used for timeout methods testing
 */
...
//===================================START RUNSTEP TIMEOUT=============================================
	...
	public void runST5() throws Exception {
		...
		/*
		 * monitored.put("carry", "WOLF"); imp.runStep(id, monitored);
		 * monitored.put("carry", "GOAT"); imp.runStep(id, monitored);
		 * monitored.put("carry", "FERRYMAN"); imp.runStep(id, monitored);
		 */
		Thread.sleep(50);
	}
...
//===================================START RUNUNTILEMPTY TIMEOUT=======================================
	...
	public void runUEM3() throws Exception {
		...
		assertTrue(new RunOutput(Esit.UNSAFE, "the id is not found")
				.equalsMessage(imp.runUntilEmptyTimeout(-1, monitored, 500)));
	}

	@Test // input mismatch, run times out
	public void runUEM4() throws Exception {
		System.out.println(
				" |||||||||||||||||||||  TEST RUNUNTILEMPTYTMON4 |||||||||||||||||||||||||||||||||||||||||||||");
		String model = "examples/Lavatrice.asm";
		SimulationContainer imp = new SimulationContainer();
		imp.init(1);
		Map<String, String> monitored = new HashMap<String, String>();
	...
	public void runUEM7() throws Exception {
		...
		monitored.put("operation", "ALLUMER");
		assertTrue(
				new RunOutput(Esit.UNSAFE, "Run timed out").equalsMessage(imp.runUntilEmptyTimeout(id, monitored, 1)));
		Thread.sleep(50);
	}
